 ...
 *         \unit  VaCommonLib::Iterators::ZipWithIndex
 *
 *      \details  This class can replace most simple for loops by using ZipWithIndex on a container in combination
 ...
 * \tparam T Type of the value for the Iterator.
 * \tparam I Type of the index for the Iterator.
 */
 ...
 * \tparam Iterable The contained iterable type which fulfills C++ named requirements: Container.
 * \trace  CREQ-VaCommonLib-WrapperIterator
 */
template <typename Iterable>
class ZipWithIndex final {  // VCA_VAC_STD_FUNC_FULFILL_CONTRACT
  /*!
 ...
 public:
   ...
   * \param[in] iterable The iterable that is to be zipped.
  ...
  ZipWithIndex(iterable_t&& iterable, size_type offset, difference_type step) noexcept
      : iterable_{std::move(iterable)}, offset_{offset}, step_size_{step} {}  // VCA_VAC_STD_FUNC_FULFILL_CONTRACT
  ...
  ZipWithIndex(iterable_t const& iterable, size_type offset, difference_type step) noexcept
      : iterable_{iterable}, offset_{offset}, step_size_{step} {}  // VCA_VAC_STD_FUNC_FULFILL_CONTRACT
   ...
   * \tparam Value The Iterator type.
   */
  // TODO(ofawzi): typename Value is not used, this will be fixed in HUN-2887.
  ...
  class Iterator final {
   public:
     ...
     * \param[in] it The iterator that is to be zipped.
     ...
     */
    auto operator*() const noexcept -> ZipWithIndexValue<iterator_value_t, size_type> {
      // VCA_VAC_VALID_RETURN_VALUE
   ...
   * \details We use the passed iterable to create an iterator from start.
   * \pre     -
   * \spec
   *   requires true;
   * \endspec
   * \return  A ZipWithIndex iterator object.
   */
  auto begin() noexcept -> iterator { return {std::begin(this->iterable_), *this}; }

  /*!
   * \brief   Create iterator from the sentinel value.
   * \details We use the passed iterable to create an iterator with the end sentinel value.
   * \pre     -
   * \spec
   *   requires true;
   * \endspec
 ...
 * \param[in] iterable The iterable that is to be zipped which fulfills C++ named requirements: Container.
 * \param     offset The offset from which the index shall start.
 * \param     step The step size that is added after each step.
 ...
 */
template <typename Iterable>
auto zip_with_index(Iterable&& iterable, typename std::decay_t<Iterable>::size_type offset = 0,
                    typename std::decay_t<Iterable>::difference_type step = 1) noexcept -> ZipWithIndex<Iterable> {
