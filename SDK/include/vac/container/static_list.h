 ...
 *         \unit  VaCommonLib::ContainerLibrary::StaticContainers::StaticList
 *
 *********************************************************************************************************************/
 ...
 public:
  ...
  class Node final : public vac::container::IntrusiveListNode<Node> {
   public:
     ...
     * \tparam    Args Types of the elements which to put into the list.
     * \param[in] args Elements which to put into the list.
     * \pre       -
     * \spec
     *   requires true;
     * \endspec
     ...
     * \endspec
     */
    ~Node() noexcept final = default;  // VCA_VAC_SAFE_DEALLOCATION
  ...
  class iterator final {
   public:
    ...
    /*!
     * \brief     Construct an iterator from a base_iterator.
     * \param[in] it Base iterator from which to create this iterator.
     * \pre       -
     * \spec
     *   requires true;
     * \endspec
     ...
     * \endspec
     */
    reference operator*() noexcept { return it_->GetElem(); }  // VCA_VAC_VALID_RETURN_VALUE
     ...
     * \endspec
     */
    const_reference operator*() const noexcept { return it_->GetElem(); }  // VCA_VAC_VALID_RETURN_VALUE
  ...
  class const_iterator final {
   public:
    ...
    /*!
     * \brief     Construct an iterator from a base_iterator.
     * \param[in] it The base_iterator from which to construct the iterator.
     * \pre       -
     * \spec
     *   requires true;
     * \endspec
     ...
     * \endspec
     */
    reference operator*() const noexcept { return it_->GetElem(); }  // VCA_VAC_VALID_RETURN_VALUE
   ...
   * \param[in] other StaticList to be moved from.
   * \pre       -
   * \spec
   *   requires true;
   * \endspec
   ...
   * \param[in] other The StaticList to move from.
   ...
   *          new_capacity will be rejected with a Abort().
   * \details If memory cannot be allocated, default allocator which is std::allocator will
   *          throw std::bad_alloc and terminate since reserve() is noexcept.
   ...
   * \tparam    Args Types of the arguments to instantiate new 0bject.
   * \param[in] args Arguments to instantiate new object.
   * \pre       The List must not be full, otherwise ara::core::Abort will be called.
   */
  template <typename... Args>
  ...
  T& front() noexcept {
    ...
    return queue_.front().GetSelf()->GetElem();  // VCA_VAC_VALID_RETURN_VALUE
  }
   ...
   * \brief     Insert an element at the back of the list.
   * \details   Terminates if the constructor of the new element throws.
   * \param[in] value Object to move into the list.
   * \pre       The List must have capacity for one more element, otherwise ara::core::Abort will be called.
   */
   ...
   * \details   Terminates if the constructor of the new element throws.
   * \tparam    Args Types of the arguments to instantiate new object.
   * \param[in] args Arguments to instantiate new object.
   * \pre       The List must have capacity for one more element, otherwise ara::core::Abort will be called.
   */
  template <typename... Args>
  ...
  T& back() noexcept {
    // undefined behavior on empty list.
    return queue_.back().GetSelf()->GetElem();  // VCA_VAC_VALID_RETURN_VALUE
  }
 ...
 * \brief  Type for nodes of the static list.
 *         Derive from this type to create an object that can be a list member.
 * \tparam T The type of the list object.
 */
template <typename T>
