 ...
 *         \unit  VaCommonLib::ContainerLibrary::ViewContainers::ContainerAggregator
 *
 ...
 * \tparam  ContainerPointer The type of container pointer. Can be raw pointer, unique_ptr, shared_ptr.
 *          The pointed-to container type must fulfill the "C++ named requirements: Container".
 * \trace   CREQ-VaCommonLib-ContainerAggregator
 */
template <typename ContainerPointer>
class ContainerAggregator final {
 public:
  /*!
   * \brief Pointer type to containers.
   */
  ...
  class const_iterator final {
   public:
     ...
     * \endspec
     * \internal
     *              - #10 For equality, the container iterators must point to the same container and the element
     *                    iterators must point to the same element.
     * \endinternal
     */
     ...
     * \internal
     *              - #10 Check if there are containers after the current container iterator.
    ...
    const_iterator& operator++() noexcept {
      ...
      if (container_iter_ != container_end_) {
        ...
        if (element_iter_ == (*container_iter_)->cend()) {  // VCA_VAC_VALID_NONSTATIC_METHOD
          bool done{false};
          // #21 Go to the next container containing at least one element.
          do {
            ++container_iter_;
          ...
          if (!done) {
            // VECTOR NL AutosarC++17_10-M0.3.1: MD_VAC_M0.3.1_pointerIsNullAtDereference
            element_iter_ = (*container_iter_)->cbegin();  // VCA_VAC_VALID_NONSTATIC_METHOD
          }
        }
   ...
   private:
    ...
    /*! \brief Current iterator of a container element. */
  ...
  std::size_t size() const noexcept {
    return std::accumulate(
        ...
        // VECTOR NL AutosarC++17_10-A18.1.5: MD_VAC_A18.1.5_constStdUniquePtrRefAsParam
  ...
  const_iterator cbegin() const noexcept {
    ...
    for (typename ara::core::Span<ContainerPointer const>::const_iterator iter{registered_containers_.cbegin()};
      ...
      if ((**iter).size() > 0) {
        ret_value = const_iterator(registered_containers_, iter, (**iter).cbegin());  // VCA_VAC_VALID_NONSTATIC_METHOD
        break;
      }
    }
    // #20 Return cend() if all containers are empty.
   ...
   * \return An iterator pointing to the sentinel end value.
   */
  static const_iterator end() noexcept { return cend(); }
