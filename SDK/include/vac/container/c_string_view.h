 ...
 *         \unit  VaCommonLib::ContainerLibrary::ViewContainers::CStringView
 *
 ...
 * \tparam Char The type of character encoding.  Only char should be used.
 * \trace  CREQ-VaCommonLib-CStringView
 */
...
class BasicCStringView final {
 public:
  ...
  BasicCStringView(const_pointer data, size_type length) noexcept
      : BasicCStringView(ara::core::in_place, data, length) {
    // #20 Check that the data pointer is valid
    if (data == nullptr) {
      ara::core::Abort(
   ...
   *            can be extracted.
   * \tparam    DataType The type of the contained contiguous data.
   * \param     container A container providing contiguous data. The contained data must be a valid null-terminated
   *            c-string. Otherwise ara::core::Abort is called.
   */
   ...
   * \details This constructor is used because std::string violates the assumption of the templated constructor.
   *          For std::string, size() does not include the null terminator byte.
   ...
   * \param[in,out] that The string view to swap.
   * \return        void.
   * \pre           -
   * \spec
   *   requires true;
  ...
  constexpr static auto FromLiteral(Char const* data, std::size_t length) noexcept -> BasicCStringView<Char> {
    return {ara::core::in_place, data, length + 1};
  }
  ...
  static auto FromString(std::basic_string<Char, std::char_traits<Char>, Allocator> const& basic_string) noexcept
    ...
    return {ara::core::in_place, basic_string.c_str(), basic_string.size() + 1};
  }
   ...
   * \tparam   Allocator The type of the allocator for string instantiation.
   * \param    alloc Allocator for string instantiation.
   * \return   The created string.
   * \pre      -
   * \spec
 ...
 * \tparam        CharT The type of character encoding.
 * \param[in,out] os The output stream to write in.
...
auto operator<<(StreamT& os, BasicCStringView<CharT> const& sv) noexcept(false)
