 ...
 *       \unit  VaCommonLib::LanguageSupport::Backports::Cpp20Backport
 ...
 * \tparam Container The type of container. Container must be a type with a size() method.
...
constexpr auto ssize(Container const& c) noexcept
    -> std::common_type_t<std::ptrdiff_t, std::make_signed_t<decltype(c.size())>> {
  /*! \brief Type alias.
   */
  using SignedType = std::common_type_t<std::ptrdiff_t, std::make_signed_t<decltype(c.size())>>;
 ...
 *  \tparam T The type to define type identity.
 */
template <typename T>
struct type_identity {
  /*! \brief Identity type. */
  using type = T;  // VECTOR SL Linker-Tolerated_Type_Difference: MD_VAC_Linker-Tolerated_Type_Difference
};
