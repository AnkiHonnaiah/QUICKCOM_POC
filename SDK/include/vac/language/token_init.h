 ...
 *         \unit  VaCommonLib::ErrorHandling::Token
 *
 ...
 * \param[in]  pre Pre-construction function.
 * \param[in]  args Arguments for the pre-construction function.
 * \pre        -
 * \return     The constructed instance of T, or the error returned by the pre-construction function.
 * \error      PreConstr(Args&&...)::error_type, Any error that is returned by T::Preconstruct(Args&&...).
 * \spec
 *  requires true;
 * \endspec
 * \trace      CREQ-VaCommonLib-TwoPhaseConstruction
 */
template <typename T, typename PreConstr, typename... Args>
auto TokenInitCustom(PreConstr&& pre, Args&&... args) noexcept
    -> ara::core::Result<T, typename std::result_of_t<PreConstr(Args&&...)>::error_type> {
  /*!
   * \brief An alias for result_of_t<PreConstr(Args && ...)>.
   */
  using PreConstrResult = std::result_of_t<PreConstr(Args && ...)>;
...
auto TokenInit(Args&&... args) noexcept
    -> ara::core::Result<T, typename std::result_of_t<decltype (&T::Preconstruct)(Args&&...)>::error_type> {
