 ...
 *         \unit  VaCommonLib::LanguageSupport::FunctionRef
 *
...
class StorageAndOperator {
  /*! \brief An alias for adding ConditionalConst for void type. */
  using VoidC = typename AddConditionalConst<void, IsConst>::type;
 ...
 public:
  ...
  R operator()(Args... args) const noexcept {
    return erased_function_(erased_object_, std::forward<Args>(args)...);  // VCA_VAC_VALID_FUNC_CALL
  }
   ...
   * \param[in,out] other The other StorageAndOperator to swap with.
   * \pre           -
   * \spec
   *  requires true;
   * \endspec
   ...
   * \param[in] fun_obj The function object pointer to be saved.
   * \pre       -
   * \spec
   *  requires true;
   * \endspec
   */
  template <typename Callable, typename = std::enable_if_t<!std::is_pointer<std::decay_t<Callable>>::value>>
  void Bind(Callable& fun_obj) noexcept {
    /*! \brief Callable type with conditional const qualifier. */
    erased_object_.fun_obj = static_cast<VoidC*>(std::addressof(fun_obj));
    erased_function_ = [](callable_ptr_t ecall, Args... args) -> R {  // VCA_VAC_LAMBDA
      // The explicit cast to R avoids a compile error when R is void and the callable returns non-void.
      using CallableC = typename AddConditionalConst<std::remove_reference_t<Callable>, IsConst>::type;
      // VCA_VAC_VALID_NONSTATIC_METHOD, VCA_VAC_FUNC_FULFILL_CONTRACT_RUN_TIME_CHECK
      return static_cast<R>((*static_cast<CallableC*>(ecall.fun_obj))(std::forward<Args>(args)...));
    };
  }
   ...
   * \param[in] fun_ptr The function pointer to be saved.
   * \pre       -
   * \spec
   *  requires true;
   * \endspec
  ...
  callable_ptr_t erased_object_;

  // VECTOR Next Construct VectorC++-V11.0.2: MD_VAC_V11-0-2_mutableMemberShallBePrivate
  /*! \brief Pointer to the callable. */
  R (*erased_function_)(callable_ptr_t, Args...);
};
...
/*! \brief  Expands the argument type pack and provides the fully specified StorageAndOperator type.
 *  \tparam SigProps Signature properties received from detail::signature::GetSigProps.
 *  \tparam Args A tuple type with all the arguments for calling the callable.
 */
...
class StorageAndOperatorSelectorArgExpander<SigProps, signature::SignatureArgs<Args...>> {
 ...
 public:
  ...
  using type = StorageAndOperator<InvokeAsConst, typename SigProps::RetT, Args...>;
};

/*!
 * \brief  Provides the StorageAndOperator type using some indirections.
 * \tparam Signature Signature properties received from detail::signature::GetSigProps.
 */
...
class StorageAndOperatorSelector {
 ...
 public:
  /*! \brief Resolved type.
   */
  using type = typename StorageAndOperatorSelectorArgExpander<SigProps, typename SigProps::ArgsT>::type;
};
...
class FunctionRef : private detail::StorageAndOperatorSelector<Signature>::type {
 ...
 public:
  ...
  // FunctionRef is neither meant to perform move construction nor move assignment.
  // But the move constructor is not explicitly declared as deleted as this causes issues with overload resolution when
  // initializing FunctionRef with r-values.
  //
  // Instead the move constructor is implicitly deleted by providing the default implementation (which will never be
  // called).
   ...
   * \details   The move constructor is never called but needs to be accessible to perform copy construction.
   *            The move constructor is needed pre C++17 to allow FunctionRef function parameter initialization
   *            from r-value callables.
   * \param[in] other The FunctionRef to move from.
   * \spec
   *  requires true;
   * \endspec
   */
   ...
   *            The callable must not be a nullptr otherwise Abort will be called.
  ...
  FunctionRef(F&& callable) noexcept
      : detail::StorageAndOperatorSelector<Signature>::type() {  // NOLINT(runtime/explicit)
    bool const is_null_ptr{detail::IsCallableNullPtr(callable)};
    if (is_null_ptr) {
      amsr::core::Abort("vac::language::FunctionRef: The callable cannot be a nullptr!");
    }
  ...
  /*!
   * \brief         Swap the content of *this with the content of other.
   * \param[in,out] other The other FunctionRef to swap with.
   * \pre           -
   * \spec
   *  requires true;
   * \endspec
