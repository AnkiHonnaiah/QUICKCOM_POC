 ...
 *         \unit  VaCommonLib::MemoryManagement::IntrusiveSharedPtr
 *
...
/*!
 * \brief  Base class of IntrusiveSharedPtr.
 * \tparam T The type of the callable for shared ptr.
 */
template <class T>
...
class IntrusiveShared {
 public:
   ...
   * \endspec
   */
  virtual ~IntrusiveShared() noexcept = default;  // VCA_VAC_SAFE_DEALLOCATION
  ...
  refcount_t DecrementReferenceCount() noexcept {
    ...
    if (refcount == 0) {
      CallDeleter();  // VCA_VAC_VALID_FUNC_CALL
    }
    return refcount;
  }
 ...
 * \tparam T The type of the callable for shared ptr.
 * \trace  CREQ-VaCommonLib-IntrusiveSharedPointer
 */
template <class T>
class IntrusiveSharedPtr final {
 public:
   ...
   * \param[in] IntrusiveShared_object The IntrusiveShared object to be constructed from.
   * \pre       -
   * \spec
   *  requires true;
   * \endspec
   ...
   * \param[in] rhs The IntrusiveShared to be assigned from.
   * \pre       -
   * \return    This IntrusiveSharedPtr object.
   * \spec
   *  requires true;
   ...
   * \param[in] rhs The IntrusiveSharedPtr to be assigned from.
   * \pre       -
   * \spec
   *  requires true;
   * \endspec
   ...
   * \param[in] rhs The IntrusiveSharedPtr to be move assigned from.
   * \pre       -
   * \return    This IntrusiveSharedPtr object.
   * \spec
   *  requires true;
   ...
   * \param[in,out] other The IntrusiveSharedPtr to be swapped.
   * \pre           -
   * \spec
   *  requires true;
   * \endspec
