 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**       \file   someip_client_event_impl.h
 *        \brief  The binding client event implementation for handling SOME/IP events subscription and reception.
...
#ifndef LIB_SOMEIP_BINDING_CORE_INCLUDE_AMSR_SOMEIP_BINDING_CORE_INTERNAL_EVENTS_SOMEIP_CLIENT_EVENT_IMPL_H_
#define LIB_SOMEIP_BINDING_CORE_INCLUDE_AMSR_SOMEIP_BINDING_CORE_INTERNAL_EVENTS_SOMEIP_CLIENT_EVENT_IMPL_H_
 ...
 */
class SomeIpClientEventImpl final : public EventAccessorInterface {
 public:
  /*!
   * \brief Constructor
   *
   ...
   * \steady  FALSE
   */
  SomeIpClientEventImpl(::amsr::someip_binding_core::internal::ProxyBindingIdentity const& proxy_identity,
   ...
   * \vprivate
   */
  ~SomeIpClientEventImpl() noexcept final;

  SomeIpClientEventImpl(SomeIpClientEventImpl const&) = delete;
  SomeIpClientEventImpl(SomeIpClientEventImpl&&) = delete;
  SomeIpClientEventImpl& operator=(SomeIpClientEventImpl const&) & = delete;
  SomeIpClientEventImpl& operator=(SomeIpClientEventImpl&&) & = delete;
 ...
 private:
  ...
  ::amsr::someip_binding_core::internal::logging::AraComLogger logger_{
      ...
      ::amsr::someip_binding_core::internal::logging::LoggerPrefixGenerator::GetLoggerPrefix("SomeIpClientEventImpl"_sv,
                                                                                             "EventId"_sv, event_id_)};
};
}  // namespace events
}  // namespace internal
}  // namespace someip_binding_core
...
#endif  // LIB_SOMEIP_BINDING_CORE_INCLUDE_AMSR_SOMEIP_BINDING_CORE_INTERNAL_EVENTS_SOMEIP_CLIENT_EVENT_IMPL_H_

/*!
 * \exclusivearea ::amsr::someip_binding_core::internal::SomeIpClientEventImpl::client_event_lock_
 *                Ensures consistency while read and write access to the is_subscribed_ flag.
 *
 * \protects ::amsr::someip_binding_core::internal::SomeIpClientEventImpl::is_subscribed_
 *           is the protected resource and contains the state if an event is subscribed or not.
 *
 * \usedin SubscribeEvent(write/read), UnsubscribeEvent(write/read), ReadSamples(read), OnEvent(read),
