 ...
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file
 *        \brief  View to a readable memory segment.
 *
 *      \details  -
 *
 *********************************************************************************************************************/

#ifndef LIB_COMMON_LIBOSABSTRACTION_INTERFACE_INCLUDE_AMSR_MEMORY_READABLE_MEMORY_VIEW_H_
#define LIB_COMMON_LIBOSABSTRACTION_INTERFACE_INCLUDE_AMSR_MEMORY_READABLE_MEMORY_VIEW_H_
 ...
 */
class ReadableMemoryView final {
 public:
  /*!
   ...
   * \endspec
   */
  constexpr ReadableMemoryView() noexcept : ReadableMemoryView(::amsr::core::Span<std::uint8_t const>{}) {}

  /*!
   * \brief       Creates a view that points to a readable memory segment.
   *
   * \param[in]   view_to_memory  Span that points to the readable memory segment.
   *
   * \context     ANY
   *
   * \pre         -
   ...
   * \steady      TRUE
   */
  explicit constexpr ReadableMemoryView(::amsr::core::Span<std::uint8_t const> view_to_memory) noexcept
      : view_{view_to_memory} {}

  /*!
   * \brief       Returns view to the readable memory segment in form if a span.
   *
   ...
   * \endspec
   */
  constexpr auto GetSpan() const noexcept -> ::amsr::core::Span<std::uint8_t const> { return view_; }

  /*!
   * \brief       Creates a sub-view of this readable memory view from an offset with a specified size.
   *
   ...
   * \steady      TRUE
   */
  auto GetSubView(Offset offset, SizeBytes size) const noexcept -> ReadableMemoryView {
    return ReadableMemoryView{view_.subspan(offset.value, size.value)};
  }

  /*!
   * \brief       Creates a sub-view of this readable memory view from an offset to the end of this view.
   *
   * \param[in]   offset  Offset (in bytes) into this view where the created view shall be start. Must be in range of
   *                      this view.
   *
   ...
   * \steady      TRUE
   */
  auto GetSubView(Offset offset) const noexcept -> ReadableMemoryView {
    return ReadableMemoryView{view_.subspan(offset.value)};
  }

  /*!
   * \brief       Interpret the memory that this view points to as an instance of a data type.
   *
   * \details     This function asserts that the size of this view and the alignment of this view allow interpreting
   *              the memory segment as such an instance of a data type.
   *
   * \tparam      T      Data type of the array that the memory should be interpreted as.
   *                     Restricted to trivially copyable, not empty type.
   *
   * \param[in]   alignment_requirement  Minimum alignment that an instance of the data type needs to have, defaults
   *                                     to alignof(T). Must be a power of 2. This view must be aligned to at least
   *                                     the specified alignment requirement.
   *
   * \return      Readable typed memory view of the data type.
   *
   * \context     ANY
   *
   ...
   *
   * \details     This function asserts that the size of this view and the alignment of this view allow interpreting
   *              the memory segment as such a data type array.
   *
   * \tparam      T      Data type of the array that the memory should be interpreted as.
   ...
   * \param[in]   alignment_requirement  Minimum alignment that an instance of the data type needs to have, defaults
   *                                     to alignof(T). Must be a power of 2.
   *                                     This view must be aligned to at least the specified alignment requirement.
   *
   * \return      Readable typed memory view of the data type.
   *
   * \context     ANY
   ...
   *  - Abort if the alignment requirement is larger than the alignment of the memory segment that this view points
   *    to.
  ...
  auto ReinterpretAsTyped(Size count, Alignment alignment_requirement = Alignment{alignof(T)}) const noexcept
    ...
    return ReadableTypedMemoryView<T>{
        // VECTOR NL AutosarC++17_10-A5.2.4: MD_OSA_A5.2.4_MemoryTypedViewReinterpretCast
        ::amsr::core::Span<T const>{reinterpret_cast<T const*>(view_.data()), count.value}};
  }

  /*!
   * \brief       Gets size (in bytes) of this readable view.
   *
 ...
 private:
  /*!
   * \brief   Span that points to the readable memory segment.
   */
  ::amsr::core::Span<std::uint8_t const> view_;
};

}  // namespace memory
}  // namespace amsr

#endif  // LIB_COMMON_LIBOSABSTRACTION_INTERFACE_INCLUDE_AMSR_MEMORY_READABLE_MEMORY_VIEW_H_
