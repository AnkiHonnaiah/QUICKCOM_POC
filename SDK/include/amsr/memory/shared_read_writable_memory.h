 ...
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file
 *        \brief  Shared memory object that is mapped as readable and writable.
 *
 *      \details  -
 *
 *********************************************************************************************************************/

#ifndef LIB_COMMON_LIBOSABSTRACTION_LINUX_INCLUDE_AMSR_MEMORY_SHARED_READ_WRITABLE_MEMORY_H_
#define LIB_COMMON_LIBOSABSTRACTION_LINUX_INCLUDE_AMSR_MEMORY_SHARED_READ_WRITABLE_MEMORY_H_
 ...
 */
class SharedReadWritableMemory final : public ReadWritableMemoryInterface {
 public:
  /*!
   * \brief       Creates a shared memory object that is mapped as readable and writable.
   *
   ...
   */
  SharedReadWritableMemory(internal::os_interface::MappedMemory&& shm,
                           std::shared_ptr<osabstraction::io::FileDescriptor> shm_fd,
                           AccessMode shared_access_mode) noexcept;

  /*!
   * \brief       SharedReadWritableMemory cannot be copied.
   */
  SharedReadWritableMemory(SharedReadWritableMemory const& other) noexcept = delete;

  /*!
   * \brief       Move constructs a SharedReadWritableMemory.
   * \param[in]   other   SharedReadWritableMemory to move construct from.
   * \steady      TRUE
   */
  SharedReadWritableMemory(SharedReadWritableMemory&& other) noexcept = default;

  /*!
   * \brief       SharedReadWritableMemory cannot be copied.
   */
  SharedReadWritableMemory& operator=(SharedReadWritableMemory const& other) & noexcept = delete;

  /*!
   * \brief       Move assigns a SharedReadWritableMemory.
   * \param[in]   other   SharedReadWritableMemory to move assign.
   * \return      Reference to this object.
   * \steady      TRUE
   */
  SharedReadWritableMemory& operator=(SharedReadWritableMemory&& other) & noexcept = default;
   ...
   * \steady      FALSE
   */
  ~SharedReadWritableMemory() noexcept final = default;
   ...
   *
   *              GetMemoryHandle() returns a SHM object file descriptor if called on a SharedReadWritableMemory object.
   *
...
#endif  // LIB_COMMON_LIBOSABSTRACTION_LINUX_INCLUDE_AMSR_MEMORY_SHARED_READ_WRITABLE_MEMORY_H_
