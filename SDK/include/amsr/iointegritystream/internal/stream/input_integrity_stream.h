 ...
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file
 *        \brief  Declaration of the InputIntegrityStream class.
 *         \unit  amsr::iointegritystream::internal::stream::InputIntegrityStream
 ...
 *
 *********************************************************************************************************************/
#ifndef LIB_IOINTEGRITYSTREAM_INCLUDE_AMSR_IOINTEGRITYSTREAM_INTERNAL_STREAM_INPUT_INTEGRITY_STREAM_H_
#define LIB_IOINTEGRITYSTREAM_INCLUDE_AMSR_IOINTEGRITYSTREAM_INTERNAL_STREAM_INPUT_INTEGRITY_STREAM_H_
...
/*!
 * \brief           Class that can perform integrity read
 * \details         InputIntegrityStream does nothing but only bridge API calls to the integrity provider.
 */
class InputIntegrityStream final : public amsr::stream::InputStream {
 public:
  /*!
   * \brief           Constructor
   * \param[in]       integrity_provider
   *                  Reference to the integrity provider that provide integrity read implementation.
   * \context         ANY
   * \pre             -
   * \threadsafe      TRUE
   * \reentrant       FALSE
   */
  explicit InputIntegrityStream(IntegrityProvider& integrity_provider) noexcept;
   ...
   * \reentrant       FALSE
   */
  InputIntegrityStream(InputIntegrityStream&& other) noexcept = default;
   ...
   * \reentrant       FALSE
   */
  ~InputIntegrityStream() noexcept final = default;  // VCA_IOIS_DEFAULT_DESTRUCTOR

  InputIntegrityStream(InputIntegrityStream const& other) = delete;
  InputIntegrityStream& operator=(InputIntegrityStream const& other) = delete;
  InputIntegrityStream& operator=(InputIntegrityStream&& other) = delete;
 ...
 private:
   ...
   * \reentrant       FALSE
   */
  explicit InputIntegrityStream(amsr::stream::InputStream& integrity_provider) noexcept;

  /*!
   * \brief           The integrity provider that provides the actual integrity implementation
   */
  amsr::stream::InputStream& integrity_provider_;
   ...
   */
  friend class UT__InputIntegrityStream;

  // VECTOR NC AutosarC++17_10-A11.3.1: MD_IoIntegrityStream_A11.3.1_friend_class
  /*!
   * \brief           Friend declaration for constructing InputIntegrityStream with IntegrityProvider
   */
  FRIEND_TEST(UT__InputIntegrityStream, IntegrityProvider);
};
...
#endif  // LIB_IOINTEGRITYSTREAM_INCLUDE_AMSR_IOINTEGRITYSTREAM_INTERNAL_STREAM_INPUT_INTEGRITY_STREAM_H_
