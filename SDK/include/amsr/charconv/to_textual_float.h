 ...
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file
 *        \brief  Converts a float value to its textual representation.
 *
 *********************************************************************************************************************/

#ifndef LIB_CHARCONV_VECTOR_INCLUDE_AMSR_CHARCONV_TO_TEXTUAL_FLOAT_H_
#define LIB_CHARCONV_VECTOR_INCLUDE_AMSR_CHARCONV_TO_TEXTUAL_FLOAT_H_
/**********************************************************************************************************************
 *  INCLUDES
 *********************************************************************************************************************/
#include <iomanip>
...
/*!
 * \brief Converts a float value to its textual representation.
 * \unit amsr::charconv::ToTextualFloat
 * \vprivate Product Private
 */
template <>
class ToTextual<float> final {
 public:
  /*!
   * \brief Constructs instance of class.
   * \param[in] buffer Memory segment to save textual representation.
   * \param[in] value_in Float value to be converted.
   * \context ANY
   * \pre -
   * \threadsafe TRUE
   * \reentrant FALSE
   ...
   * \steady UNKNOWN
   */
  ToTextual(amsr::core::Span<char> buffer, float value_in) noexcept
 ...
 private:
  /*!
   * \brief Converts a float value to its textual representation and writes it to buffer.
   * \param[in] fmt Formatting parameters for floating point output.
   * \return Textual representation of value.
   * \context ANY
   * \pre -
  ...
  amsr::core::Result<amsr::core::Span<char>> WriteFloatingPoint(FPFormat const fmt) noexcept {
    ...
    stream << std::setprecision(std::numeric_limits<float>::digits10 + 1);  // VCA_CHARCONV_SETPRECISION_FUNC
...
#endif  // LIB_CHARCONV_VECTOR_INCLUDE_AMSR_CHARCONV_TO_TEXTUAL_FLOAT_H_
