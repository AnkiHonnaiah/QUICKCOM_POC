 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file  someip_binding_xf/internal/events/someip_event_handler.h
 *        \brief  Event handler for SOME/IP events.
 *        \unit   SomeIpBinding::SomeIpBindingXf::Communication::Events::SomeIpEventHandler
 *
 *********************************************************************************************************************/

#ifndef LIB_SOMEIP_BINDING_TRANSFORMATION_LAYER_INCLUDE_AMSR_SOMEIP_BINDING_XF_INTERNAL_EVENTS_SOMEIP_EVENT_HANDLER_H_
#define LIB_SOMEIP_BINDING_TRANSFORMATION_LAYER_INCLUDE_AMSR_SOMEIP_BINDING_XF_INTERNAL_EVENTS_SOMEIP_EVENT_HANDLER_H_
...
/*!
 * \brief Handler class for SOME/IP events.
 *
...
class SomeIpEventHandler final
 ...
 public:
   ...
   * \steady  FALSE
   */
  SomeIpEventHandler(::amsr::someip_binding_core::internal::SomeIpEventIdentity const& someip_event_identity,
  ...
  ~SomeIpEventHandler() noexcept final {
    if (IsSubscribed()) {
  ...
  SomeIpEventHandler(SomeIpEventHandler const&) = delete;
  SomeIpEventHandler& operator=(SomeIpEventHandler const&) = delete;
  SomeIpEventHandler(SomeIpEventHandler&&) = delete;
  SomeIpEventHandler& operator=(SomeIpEventHandler&&) = delete;
 ...
 private:
  ...
  /*!
   * \brief        Handle a received event sample of Some/Ip message.
   * \details      Called upon reception of an event sample.
   *               Enqueues the received event in the invisible cache.
   *               Notifies the subscribed EventManager. Called from SomeipBinding.
   * \param[in]    event_message A complete Some/Ip message.
  ...
  void OnEvent(::amsr::someip_binding_core::internal::events::EventMessage const& event_message) noexcept final {
    // PTP-B-SomeIpDaemon-ProxyEventXF_OnEvent
    if (IsSubscribed()) {
      ::amsr::someip_binding_core::internal::DataBufferSharedPtr const event_message_buffer{event_message.GetBuffer()};

      amsr::core::Optional<::amsr::someip_binding_core::internal::events::EventMessage::TimeStamp> const ts_optional{
          event_message.GetTimeStamp()};

      // VCA_SOMEIPBINDING_TRIVIAL_FUNCTION_CONTRACT
      bool const sample_dropped{invisible_sample_cache_.Enqueue(event_message_buffer, ts_optional)};
      if (sample_dropped) {
        logger_.LogDebug([](::ara::log::LogStream& s) { s << "Sample dropped during OnEvent()."; },
                         static_cast<char const*>(__func__), __LINE__);
      }
      ...
      // PTP-E-SomeIpDaemon-ProxyEventXF_OnEvent
    } else {
      logger_.LogFatalAndAbort(
          [](::amsr::someip_binding_core::internal::logging::StringStream& s) {
            s << "OnEvent() called while not being subscribed!";
          },
          static_cast<char const*>(__func__), __LINE__);
    }
  }
  ...
  /*!
   * \brief Must never be called, PDU event that cannot be handled by this class.
   *
  ...
  void OnPduEvent(::amsr::someip_binding_core::internal::events::PduMessage const& pdu_message) noexcept final {
    static_cast<void>(pdu_message);
    logger_.LogFatalAndAbort(
        [](::amsr::someip_binding_core::internal::logging::StringStream& s) {
          s << "PDU event on SOMEIP event handler.";
        },
        static_cast<char const*>(__func__), __LINE__);
  }
  ...
  ::amsr::core::String const logger_prefix_{
      someip_binding_core::internal::logging::LoggerPrefixGenerator::GetLoggerPrefix(
          "SomeIpEventHandler"_sv, someip_event_identity_.GetServiceId(), someip_event_identity_.GetInstanceId(),
...
#endif  // LIB_SOMEIP_BINDING_TRANSFORMATION_LAYER_INCLUDE_AMSR_SOMEIP_BINDING_XF_INTERNAL_EVENTS_SOMEIP_EVENT_HANDLER_H_
