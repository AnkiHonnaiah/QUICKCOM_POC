...
#ifndef LIB_SOMEIP_DAEMON_CORE_INCLUDE_AMSR_SOMEIP_DAEMON_CORE_SERVICE_DISCOVERY_STATE_MACHINE_SERVER_SERVICE_DISCOVERY_SERVER_STATE_WAIT_H_
#define LIB_SOMEIP_DAEMON_CORE_INCLUDE_AMSR_SOMEIP_DAEMON_CORE_SERVICE_DISCOVERY_STATE_MACHINE_SERVER_SERVICE_DISCOVERY_SERVER_STATE_WAIT_H_
 ...
 * \trace SPEC-10144674
 */
class ServiceDiscoveryServerStateWait final : public ServiceDiscoveryServerStateInterface {
 public:
  /*!
   * \brief ServiceDiscoveryServerStateWait constructor.
   *
   * \pre          -
   * \context      Init
   * \reentrant    FALSE
   ...
   * \steady       FALSE
   */
  ServiceDiscoveryServerStateWait() noexcept;
  ...
  ~ServiceDiscoveryServerStateWait() noexcept override = default;

  ServiceDiscoveryServerStateWait(ServiceDiscoveryServerStateWait const&) = delete;
  ServiceDiscoveryServerStateWait(ServiceDiscoveryServerStateWait&&) = delete;
  ServiceDiscoveryServerStateWait& operator=(ServiceDiscoveryServerStateWait const&) & = delete;
  ServiceDiscoveryServerStateWait& operator=(ServiceDiscoveryServerStateWait&&) & = delete;
  ...
  // VECTOR NC AutosarC++17_10-A10.3.3: MD_SomeIpDaemon_AutosarC++17_10-A10.3.3_no_virtual_functions_in_final_class
  /*!
   * \brief Requests a state change to either main or repetition phase (based on configured REPETITION_MAX).
   *        Called once OfferService message is sent.
   *
   * \param[in] context The server state machine context.
   *
   * \pre -
   ...
   * \steady      TRUE
   *
   * \trace SPEC-4981643, SPEC-4981648, SPEC-10144682, SPEC-10144687, SPEC-10144688
   */
 ...
 private:
  ...
  someip_daemon_core::logging::AraComLogger const logger_{
      ...
      ara::core::StringView{"ServiceDiscoveryServerStateWait"}};
};
...
#endif  // LIB_SOMEIP_DAEMON_CORE_INCLUDE_AMSR_SOMEIP_DAEMON_CORE_SERVICE_DISCOVERY_STATE_MACHINE_SERVER_SERVICE_DISCOVERY_SERVER_STATE_WAIT_H_
