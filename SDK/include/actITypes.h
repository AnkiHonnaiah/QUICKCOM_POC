...
typedef actPlatformUint8 actHashAlgorithm;
# define actHA_SHA1                                                   (0x00u)
# define actHA_SHA2_224                                               (0x01u)
# define actHA_SHA2_256                                               (0x02u)
# define actHA_SHA2_384                                               (0x03u)
# define actHA_SHA2_512                                               (0x04u)
# define actHA_SHA2_512_224                                           (0x05u)
# define actHA_SHA2_512_256                                           (0x06u)
 /* The SHA3 identifiers need to be direct successors to each other! This is used in vSecPrim_initHash(). */
# define actHA_SHA3_224                                               (0x07u)
# define actHA_SHA3_256                                               (0x08u)
# define actHA_SHA3_384                                               (0x09u)
# define actHA_SHA3_512                                               (0x0Au)
/* The SHAKE identifiers need to be direct successors to each other! This is used in vSecPrim_initHash(). */
# define actHA_SHAKE_128                                              (0x0Bu)
# define actHA_SHAKE_256                                              (0x0Cu)
# define actHA_RMD160                                                 (0x0Du)
# define actHA_MD5                                                    (0x0Eu)
 ...
 * KDF2
 ****************************************/
# ifdef actHASH_SIZE_SHA_KDF2 /* COV_VSECPRIM_STD_VALUES */
# else
#  if VSECPRIM_ACTKDF2HMACSHA256_ENABLED /* COV_VSECPRIM_NO_SAFE_CONFIG XF */
#   define actHASH_SIZE_SHA_KDF2                                      actHASH_SIZE_SHA256
#  else
#   define actHASH_SIZE_SHA_KDF2                                      actHASH_SIZE_SHA
#  endif
# endif
...
# ifdef actHASH_SIZE_SHA_HKDF /* COV_VSECPRIM_STD_VALUES */
# else
#  if VSECPRIM_ACTHKDFHMACSHA384_ENABLED
#   define actHASH_SIZE_SHA_HKDF                                      actHASH_SIZE_SHA384
#  else
#   define actHASH_SIZE_SHA_HKDF                                      actHASH_SIZE_SHA256
#  endif
# endif

# if ((VSECPRIM_ACTHKDFHMACSHA256_ENABLED == STD_ON) || (VSECPRIM_ACTHKDFHMACSHA384_ENABLED == STD_ON)) /* COV_VSECPRIM_NO_SAFE_CONFIG TF tf xf */
#  define VSECPRIM_ACTHKDF_ENABLED                                    STD_ON
# else
#  define VSECPRIM_ACTHKDF_ENABLED                                    STD_OFF
# endif

 /****************************************
 * HMAC SHA IDs for HDF AND KDF2
 ****************************************/
/* Do not change this order! These values are used to access actHashMACInitTable,
 * actHashMACUpdateTable and actHashMACFinalizeTable. */
# define actHASHMACSHA1_ALGO_ID                                       (0u)
# define actHASHMACSHA256_ALGO_ID                                     (1u)
# define actHASHMACSHA384_ALGO_ID                                     (2u)
...
/* roundUpBNDigit takes input in bits and puts out an BNDigit of the length needed */
# define conversionFromBitsToBNDigit(x)                               (((x/8u) + (sizeof(actBNDIGIT) - 1u)) / sizeof(actBNDIGIT))
 ...
 ****************************************/
# define actEDWARDSCURVE25519                                         (25519u)
# define actEDWARDSCURVE448                                           (448u)

# define actEd25519_KEY_SIZE                                          (32u)
# define actEd25519pure                                               (0u)
# define actEd25519PURE                                               actEd25519pure
# define actEd25519ctx                                                (1u)
# define actEd25519CTX                                                actEd25519ctx
# define actEd25519ph                                                 (2u)
# define actEd25519PH                                                 actEd25519ph
# define actX25519_KEY_SIZE                                           (32u)


# define actEd448_KEY_SIZE                                            (57u)
# define actEd448_SIGNATURE_SIZE                                      (114u)
# define actEd448PURE                                                 (3u)
# define actEd448PH                                                   (4u)
# define actX448_KEY_SIZE                                             (56u)

# define actEDWARDS448_DOM4MIDDLEPART_LENGTH                          (2u)
# define actEDWARDS448_DOM4MIDDLEPART_PHFLAGIDX                       (0u)
# define actEDWARDS448_DOM4MIDDLEPART_CTXLENGTHIDX                    (1u)
# define actEDWARDS448PURE_PHFLAG                                     (0u)
# define actEDWARDS448PH_PHFLAG                                       (1u)
# define actEDWARDS448_DEFAULTCONTEXTLEN                              (0u)

# define BNDIGITS_256                                                 (256u /8u /sizeof (actBNDIGIT))
# define BNDIGITS_456                                                 conversionFromBitsToBNDigit(456u)
# define BNDIGITS_512                                                 (512u /8u /sizeof (actBNDIGIT))
# define BNDIGITS_912                                                 conversionFromBitsToBNDigit(912u)
# define BNDIGITS_912_MULTIPLICATION                                  (2u*BNDIGITS_456)
# define BNDIGITS_928                                                 (928u /8u /sizeof (actBNDIGIT))
# define BNBYTES_256                                                  (256u /8u /sizeof (actU8))
# define BNBYTES_456                                                  (456u /8u /sizeof (actU8))
# define BNBYTES_512                                                  (512u /8u /sizeof (actU8))
# define MASK_MSBit                                                   (0x7FFFFFFFuL >> (32u - actBN_BITS_PER_DIGIT))

# define actED448_SHAKE_DIGEST_LENGTH                                 (114u)
# define actED448PH_DIGESTLENGTH                                      (64u)
# define actED448_SHAKESTRENGTH                                       (256u)

# if (VSECPRIM_CURVE448_ENABLED == STD_ON)
#  define actEDWARDS_MAX_BNDIGITS                                     (BNDIGITS_456)
#  define actEDWARDS_MAX_SIGNATURE_KEY_SIZE                           (actEd448_KEY_SIZE)

#  define actEDWARDS_MAX_KEY_EXCHANGE_PRIVATE_KEY_SIZE                (actX448_KEY_SIZE)
/* Technically/By specification the curve448 based EdDH key size is 56 bytes.
 * However, all other buffers which are used to store temporary results of arithmetic
 * functions (tempA - tempH) provide at least 57 bytes of memory. In order to be compatible
 * to the implementation of these functions (which are also used for EdDSA), an additional
 * byte is provided here. */
#  define actEDWARDS_MAX_KEY_EXCHANGE_UCOORD_SIZE                     (actX448_KEY_SIZE + 1u)
#  define actEDWARDS_MAX_HASH_DIGEST_SIZE                             (actED448_SHAKE_DIGEST_LENGTH)

# else
#  define actEDWARDS_MAX_BNDIGITS                                     (BNDIGITS_256)
#  define actEDWARDS_MAX_SIGNATURE_KEY_SIZE                           (actEd25519_KEY_SIZE)
#  define actEDWARDS_MAX_KEY_EXCHANGE_PRIVATE_KEY_SIZE                (actX25519_KEY_SIZE)
#  define actEDWARDS_MAX_KEY_EXCHANGE_UCOORD_SIZE                     (actX25519_KEY_SIZE)
#  define actEDWARDS_MAX_HASH_DIGEST_SIZE                             (actHASH_SIZE_SHA512)
# endif
...
/* Workspace structure for HashMAC SHA-512 */
typedef struct
{
  actSHA512STRUCT sha512;
  actU8 key_buf[actHASH_BLOCK_SIZE_SHA512];
  actLengthType key_length;
} actHASHMACSHA512STRUCT;
...
typedef struct
{
  ...
  actU8 U_buf[actHASH_SIZE_SHA_KDF2];
  actU8 F_buf[actHASH_SIZE_SHA_KDF2];
} actKDF2STRUCT;
...
# if VSECPRIM_ACTHKDFHMACSHA384_ENABLED == STD_ON /* COV_VSECPRIM_NO_SAFE_CONFIG XF */
  actHASHMACSHA384STRUCT hmac_sha384;
# endif
  actHASHMACSHA256STRUCT hmac_sha256;
...
typedef struct
{
  ...
  { /* PRQA S 0750 */ /* MD_MSR_Union */
     ...
     * However, the key is automatically padded with zeros to be of size actHASH_BLOCK_SIZE_SHA*** in
     * actHashMACSHA***Init(). Thus, a smaller temporary key suffices in order to save memory. */
    actU8 tmpKey[actHASH_SIZE_SHA_HKDF];
    actU8 blockT[actHASH_SIZE_SHA_HKDF];
  }tmpBlock;
  actU8 keyDerivationKey[actHASH_SIZE_SHA_HKDF];
  actU8 methodID;
}actHKDFSTRUCT;

/* Workspace for HKDF Hash */

typedef union
{ /* PRQA S 0750 */ /* MD_MSR_Union */
# if (VSECPRIM_HKDF_SHA2_512_ONESTEP_ENABLED == STD_ON)
  actSHA512STRUCT SHA512ws;
# endif
  actSHA256STRUCT SHA256ws;
} actWorkSpaceUnionHashFunctions_HKDF;

typedef struct
{
  actWorkSpaceUnionHashFunctions_HKDF actwsSha;
# if (VSECPRIM_HKDF_SHA2_512_ONESTEP_ENABLED == STD_ON)
  actU8 blockT[actHASH_SIZE_SHA512];
# else
...
/* Big Number Types */
/* Curve point represented in extended homogeneous coordinates used for operations in Ed25519. */
typedef struct
{
...
} actExtendedHomogeneousPoint;

/* Curve point represented in homogeneous coordinates used for operations in Ed448. */
typedef struct
{
  actBNDIGIT X[BNDIGITS_456];
  actBNDIGIT Y[BNDIGITS_456];
  actBNDIGIT Z[BNDIGITS_456];
} actHomogeneousPointEd448;
...
/* workspace for ECC key check */
typedef struct
{
  actECPBasicStruct baseData;
} act_WorkSpaceEcPKeyCheck;
...
/* Ed25519 and Ed448 STRUCTS */
typedef actU16 actEdwardsCurveType;

/* buffers for temporary results of Ed25519, X25519, Ed448 and X448. */
typedef struct
{
  actBNDIGIT tempA[actEDWARDS_MAX_BNDIGITS];
  actBNDIGIT tempB[actEDWARDS_MAX_BNDIGITS];
  actBNDIGIT tempC[actEDWARDS_MAX_BNDIGITS];
  actBNDIGIT tempD[actEDWARDS_MAX_BNDIGITS];
  actBNDIGIT tempE[actEDWARDS_MAX_BNDIGITS];
  actBNDIGIT tempF[actEDWARDS_MAX_BNDIGITS];
  actBNDIGIT tempG[actEDWARDS_MAX_BNDIGITS];
  actBNDIGIT tempH[actEDWARDS_MAX_BNDIGITS];
  /* temp1 is used in actBNMult to save the result of the multiplication.
  We add 1 digit to the buffer since the digit length of temp1 MUST be bigger or equal length_a + length_b.
  BNDIGITS_912 does not fullfil this requirement because of rounding up operations used to transform
  bit length in digit length. 
  Therefore BNDIGITS_912_MULTIPLICATION, which is twice the length of BNDIGITS_456 */
  actBNDIGIT temp1[BNDIGITS_912_MULTIPLICATION];
# if (actBN_BITS_PER_DIGIT == 8) /* COV_VSECPRIM_NO_SAFE_CONFIG XF */
  /* one extra digit needed for act448_reduceP to avoid out ouf bound access on array */
  actBNDIGIT tempQ[actEDWARDS_MAX_BNDIGITS+1];
# else
  actBNDIGIT tempQ[actEDWARDS_MAX_BNDIGITS];
# endif
} actTempType;
...
typedef struct
{
  actBNDIGIT prime[actEDWARDS_MAX_BNDIGITS];
  actBNDIGIT order[actEDWARDS_MAX_BNDIGITS];

# if (VSECPRIM_EDDSA_448_ENABLED == STD_ON)
  /* 2^k is (k+1) bit long --> 929 bit, q = 446 bit long ==> 2^k / q is 929 - 446 = 483 bit long */
  actBNDIGIT precomputedBarrettConstant[BNDIGITS_512];
# else
  actBNDIGIT precomputedBarrettConstant[BNDIGITS_256 + 1u];
# endif

  actBNRING pRing;
  actBNRING qRing;
  actTempType auxVar;
} actEdwardsCurveWs;            /* size <= 520 bytes (64bit pointers) */

/* workspace structure for key checks on ed25519 and ed448 */
typedef struct
{
  actEdwardsCurveWs curveInformation;
# if (VSECPRIM_ECDSA_25519_ENABLED == STD_ON)
  actExtendedHomogeneousPoint point25519;
#endif 
# if (VSECPRIM_EDDSA_448_ENABLED == STD_ON)
  actHomogeneousPointEd448 point448;
#endif
  actEdwardsCurveType curveId;
} actEdwardsKeyCheck;         

/* Workspace structure for Ed25519 or Ed448 respectively */
typedef struct
{
...
# if (VSECPRIM_EDDSA_448_ENABLED == STD_ON)
  union
  {                                 /* PRQA S 0750 */ /* MD_MSR_Union */
    actKECCAKcSTRUCT shake;                            /* SHAKE-256          - used for pre-hash and Ed448 */
    actHomogeneousPointEd448 point_A;                  /* A:  need 180 bytes - actKECCAKcSTRUCT is at least 292 bytes */
    actHomogeneousPointEd448 point_rB;                 /* rB: need 180 bytes - actKECCAKcSTRUCT is at least 292 bytes */
    actHomogeneousPointEd448 point_sB;                 /* sB: sB / A, sB / B are NEVER used concurrently! */
  } shakeWs;
# endif
# if (VSECPRIM_ECDSA_25519_ENABLED == STD_ON)
  union
  {                                 /* PRQA S 0750 */ /* MD_MSR_Union */
    ...
    actExtendedHomogeneousPoint point_A;             /* A:  need 128 bytes - actSHA512Struct is 212 bytes */
    actExtendedHomogeneousPoint point_rB;            /* rB: need 128 bytes - actSHA512Struct is 212 bytes */
    actExtendedHomogeneousPoint point_sB;            /* sB: sB / A, sB / rB are NEVER used concurrently! */
  } shaWs;
# endif
  union
  {                                 /* PRQA S 0750 */ /* MD_MSR_Union */
    actU8 hashValue[actEDWARDS_MAX_HASH_DIGEST_SIZE]; /* hashed message  - used for pre-hash */
    actBNDIGIT bnDigit_s[actEDWARDS_MAX_BNDIGITS];    /* s: need  32/57 (+ 3) bytes - hashValue is at least 64 bytes */
    actBNDIGIT bnDigit_k[actEDWARDS_MAX_BNDIGITS];    /* k: k / s  are NEVER used concurrently! */
  } hashBuf;

  actU8 privateKey[actEDWARDS_MAX_HASH_DIGEST_SIZE];  /* private scalar & prefix stored one after another */

  union
  {                                 /* PRQA S 0750 */ /* MD_MSR_Union */
    actU8 publicKey[actEDWARDS_MAX_SIGNATURE_KEY_SIZE]; /* public key: 32/57 bytes (exactly) */
    actBNDIGIT bnDigit_a[actEDWARDS_MAX_BNDIGITS];      /* a: need  32/57 (+ 3) bytes */
  } pubKeyBuf;
  ...
  actEdwardsCurveWs coreWS;               /* workspace for calculation core (layout depends on core used) */
  actEdwardsCurveType curveId;
} actEdDSASTRUCT;

/* Workspace structure for X25519/X448 */
typedef struct
{
  /* For the private key, 56 bytes are sufficient. */
  actU8 privateKey[actEDWARDS_MAX_KEY_EXCHANGE_PRIVATE_KEY_SIZE];   /* private key */
  /* It would be sufficient for bnDigit_x_1 to provide storage for 448 bits instead of for 456 bits.
   * However, the EdDH implementation relies on the same arithmetic functions (addition, multiplication,...) 
   * as the EdDSA implementation. The implementation of these arithmetic functions in turn depends on
   * the fact that the provided input buffers are exactly 456 bits long.
   * Thus, bnDigit_x_1 provides an additional digit. */
  union
  { /* PRQA S 0750 */ /* MD_MSR_Union */
    actU8 uCoord[actEDWARDS_MAX_KEY_EXCHANGE_UCOORD_SIZE];     /* u (x) coordinate */
    actBNDIGIT bnDigit_x_1[actEDWARDS_MAX_BNDIGITS];        /* x_1: x_1 / u  are NEVER used concurrently! */
  } basePointBuf;
  actEdwardsCurveWs coreWS;                                 /* workspace for calculation core */
  actEdwardsCurveType curveId;
} actEdDHSTRUCT;

/* Compatibility */
typedef actEdDHSTRUCT actX25519Struct;
typedef actEdDSASTRUCT actEd25519Struct;
...
/* Workspace union for Hash in RSA */
typedef union
{  /* PRQA S 0750 */ /* MD_MSR_Union */
...
# if ( VSECPRIM_RSA_V15_RIPEMD160_ENABLED == STD_ON || VSECPRIM_RSA_PSS_RIPEMD160_ENABLED == STD_ON ) /* COV_VSECPRIM_NO_SAFE_CONFIG XF xf xf */
  actRMD160STRUCT RIPEMD160ws;
# endif
  actSHASTRUCT SHA1ws;
} actWorkSpaceUnionHashFunctions_RSA;
