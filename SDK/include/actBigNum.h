...
# define actBN_MAX_DIGIT                                              ((actBNDIGIT)(actBN_BASE - 1u))
 ...
 *  \param[in]   length          Length of a and b,
 *  \param[in]   completeRuntime Flag to indicate whether the comparison shall stop once a difference is found (FALSE)
 *                               or not (TRUE)
 ...
 *               If after actBNMODRANDOMIZE_RNG_MAX_RETRIES tries, the generated random number is still 0,
 ...
 *  \pre         Ring->m must be odd
 *               Provided pointers must be valid
 *               The buffer referenced by x must provide at least Ring->m_length actBNDIGITs.
 *               The buffers referenced by t[0] and t[1] each must provide at least (Ring->m_length + 1) actBNDIGITs.
 *  \context     TASK
 *  \reentrant   TRUE
 *  \synchronous TRUE
