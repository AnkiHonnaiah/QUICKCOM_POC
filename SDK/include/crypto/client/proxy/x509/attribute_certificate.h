 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file  proxy/x509/attribute_certificate.h
 *        \brief  Proxy implementation for ara::crypto::x509::AttributeCertificate.
 *
 *      \details  Client side implementation of the ara::crypto::x509::AttributeCertificate interface that
 *                transparently communicates with a server side implementation that performs the real cryptography.
 *
 *********************************************************************************************************************/

#ifndef LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_X509_ATTRIBUTE_CERTIFICATE_H_
#define LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_X509_ATTRIBUTE_CERTIFICATE_H_
...
/*!
 * \brief Client-side implementation for the proxy of ara::crypto::x509::AttributeCertificate.
 * \vprivate Component Private
 * \trace DSGN-Crypto-ProcessIsolation, DSGN-Crypto-Skeleton_Proxy
 */
class AttributeCertificate final : public BaseProxy, public ara::crypto::x509::AttributeCertificate {
 public:
  /*!
   * \brief Constructor
   ...
   * \vprivate Component Private
   */
  explicit AttributeCertificate(ara::crypto::cryp::CryptoProvider const& provider_reference,
      ...
      : BaseProxy{transporter, message_builder, message_serializer, message_deserializer},
        ara::crypto::x509::AttributeCertificate() {
    static_cast<void>(provider_reference);
  }
   ...
   * \vprivate Component Private
   */
  static std::unique_ptr<AttributeCertificate, ara::crypto::CustomDeleter> Create(
      ara::crypto::cryp::CryptoProvider const& provider_reference, amsr::sec::ipc::Transporter& transporter,
   ...
   * \vprivate Component Private
   */
  ~AttributeCertificate() noexcept final = default;

  /*!
   * \brief Deleted copy constructor
   * \vprivate Component Private
   */
  AttributeCertificate(AttributeCertificate const& /*other*/) = delete;

  /*!
   * \brief Deleted move constructor
   * \vprivate Component Private
   */
  AttributeCertificate(AttributeCertificate&& /*other*/) noexcept = delete;
   ...
   * \vprivate Component Private
   */
  AttributeCertificate& operator=(AttributeCertificate const& /*other*/) = delete;
   ...
   * \vprivate Component Private
   */
  AttributeCertificate& operator=(AttributeCertificate&& /*other*/) = delete;
  ...
  /*!
   * \brief Return the start time of the validity time frame of the certificate.
   * \return the certificate validity start time.
   * \context ANY
   * \pre -
   * \threadsafe TRUE
   * \reentrant TRUE
  ...
  /*!
   * \brief Return the end time of the validity time frame of the certificate.
   * \return the certificate validity end time.
   * \context ANY
   * \pre -
   * \threadsafe TRUE
   * \reentrant TRUE
...
#endif  // LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_X509_ATTRIBUTE_CERTIFICATE_H_
