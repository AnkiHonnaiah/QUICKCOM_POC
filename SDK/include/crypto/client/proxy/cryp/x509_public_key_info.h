 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file  proxy/cryp/x509_public_key_info.h
 *        \brief  Proxy implementation for ara::crypto::cryp::X509PublicKeyInfo.
 *
 *      \details  Client side implementation of the ara::crypto::cryp::X509PublicKeyInfo interface that
 *                transparently communicates with a server side implementation that performs the real cryptography.
 *
 *********************************************************************************************************************/

#ifndef LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_CRYP_X509_PUBLIC_KEY_INFO_H_
#define LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_CRYP_X509_PUBLIC_KEY_INFO_H_
...
/*!
 * \brief Client side proxy implementation for ara::crypto::cryp::X509PublicKeyInfo
 * \vprivate Component Private
 * \trace DSGN-Crypto-ProcessIsolation, DSGN-Crypto-Skeleton_Proxy
 */
class X509PublicKeyInfo : public BaseProxy, public ara::crypto::cryp::X509PublicKeyInfo {
 public:
  /*!
   ...
   * \vprivate Component Private
   */
  X509PublicKeyInfo(ara::crypto::cryp::CryptoProvider& provider_reference, amsr::sec::ipc::Transporter& transporter,
   ...
   * \vprivate Component Private
   */
  static std::unique_ptr<X509PublicKeyInfo, ara::crypto::CustomDeleter> Create(
   ...
   * \error ara::crypto::SecurityErrc::kInsufficientCapacity Buffer is too small to hold the PublicKeyInfo.
   * \reentrant TRUE
   * \synchronous TRUE
   * \vprivate Component Private
   */
   ...
   *                                             only the heap is supported.
   * \error ara::crypto::SecurityErrc::kUsageViolation OID is empty.
   * \error ara::crypto::SecurityErrc::kBadObjectReference Unqble to load domain parameters.
   * \reentrant TRUE
   * \synchronous TRUE
   * \vprivate Component Private
   */
   ...
   * \pre -
   * \threadsafe TRUE
   * \error ara::crypto::SecurityErrc::kBadObjectType Unexpected object type.
   ...
   * \error ara::crypto::SecurityErrc::kIncompatibleArguments The trusted container is invalid or not compatible.
   ...
   * \vprivate Component Private
   */
  bool IsSameKey(ara::crypto::cryp::PublicKey const& public_key) const noexcept override;
...
#endif  // LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_CRYP_X509_PUBLIC_KEY_INFO_H_
