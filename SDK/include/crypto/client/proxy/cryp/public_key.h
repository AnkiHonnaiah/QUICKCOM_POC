 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file  proxy/cryp/public_key.h
 *        \brief  Proxy implementation for ara::crypto::cryp::PublicKey.
 *
 *      \details  Client side implementation of the ara::crypto::cryp::PublicKey interface that
 *                transparently communicates with a server side implementation that performs the real cryptography.
 *
 *********************************************************************************************************************/

#ifndef LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_CRYP_PUBLIC_KEY_H_
#define LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_CRYP_PUBLIC_KEY_H_
...
/*!
 * \brief Client side proxy implementation for ara::crypto::cryp::PublicKey
 * \vprivate Component Private
 * \trace DSGN-Crypto-ProcessIsolation, DSGN-Crypto-Skeleton_Proxy
 */
class PublicKey : public BaseProxy, public ara::crypto::cryp::PublicKey {
 public:
  /*!
   * \brief Unique costant smart pointer of the proxy.
   * \vprivate Component Private
   */
  using Uptrc = std::unique_ptr<PublicKey const, ara::crypto::CustomDeleter>;
   ...
   * \vprivate Component Private
   */
  PublicKey(ara::crypto::cryp::CryptoProvider& provider_reference, amsr::sec::ipc::Transporter& transporter,
   ...
   * \vprivate Component Private
   */
  PublicKey(PublicKey const& /*other*/) = delete;

  /*!
   * \brief Deleted move constructor
   * \vprivate Component Private
   */
  PublicKey(PublicKey&& /*other*/) noexcept = delete;
   ...
   * \vprivate Component Private
   */
  static std::unique_ptr<PublicKey, ara::crypto::CustomDeleter> Create(
    ...
    return std::unique_ptr<PublicKey, ara::crypto::CustomDeleter>(
        new PublicKey(provider_reference, transporter, message_builder, message_serializer, message_deserializer));
  }
   ...
   * \vprivate Component Private
   */
  bool CheckKey(bool strong_check) const noexcept override;
   ...
   * \pre -
   * \threadsafe TRUE
   * \error ara::crypto::SecurityErrc::kIncompleteArgState Uninitialized hashing context.
   * \error ara::crypto::SecurityErrc::kInsufficientCapacity Buffer too small.
   * \error ara::crypto::SecurityErrc::kRuntimeFault Hashing failed.
   * \reentrant TRUE
   * \synchronous TRUE
   * \vprivate Component Private
   */
  ara::core::Result<std::size_t> HashPublicKey(ara::crypto::WritableMemRegion hash,
                                               ara::crypto::cryp::HashFunctionCtx& hash_func) const noexcept override;
...
#endif  // LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_CRYP_PUBLIC_KEY_H_
