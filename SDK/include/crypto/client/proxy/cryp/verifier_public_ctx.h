 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file  proxy/cryp/verifier_public_ctx.h
 *        \brief  Proxy implementation for ara::crypto::cryp::VerifierPublicCtx.
 *
 *      \details  Client side implementation of the ara::crypto::cryp::VerifierPublicCtx interface that
 *                transparently communicates with a server side implementation that performs the real cryptography.
 *
 *********************************************************************************************************************/

#ifndef LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_CRYP_VERIFIER_PUBLIC_CTX_H_
#define LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_CRYP_VERIFIER_PUBLIC_CTX_H_
...
/*!
 * \brief Client side proxy implementation for ara::crypto::cryp::VerifyPubulicCtx
 * \vprivate Component Private
 * \trace DSGN-Crypto-ProcessIsolation, DSGN-Crypto-Skeleton_Proxy
 */
class VerifierPublicCtx : public BaseProxy, public ara::crypto::cryp::VerifierPublicCtx {
 public:
  /*!
   * \brief Unique smart pointer of the proxy.
   * \vprivate Component Private
   */
  using Uptr = std::unique_ptr<VerifierPublicCtx, ara::crypto::CustomDeleter>;
   ...
   * \vprivate Component Private
   */
  VerifierPublicCtx(ara::crypto::cryp::CryptoProvider& provider_reference, amsr::sec::ipc::Transporter& transporter,
   ...
   * \vprivate Component Private
   */
  static std::unique_ptr<VerifierPublicCtx, ara::crypto::CustomDeleter> Create(
    ...
    return std::unique_ptr<VerifierPublicCtx, ara::crypto::CustomDeleter>(new VerifierPublicCtx(
  ...
  /*!
   * \brief Check if the verifier context is keyed.
  ...
  /*!
   * \brief Clear the verifier context and set the domain parameters.
   ...
   * \vprivate Component Private
   */
  ara::core::Result<void> SetKey(ara::crypto::cryp::PublicKey const& key) noexcept override;
   ...
   * \vprivate Component Private
   */
  ara::core::Result<bool> Verify(ara::crypto::cryp::HashFunctionCtx const& hash,
                                 ara::crypto::cryp::Signature const& signature,
   ...
   * \vprivate Component Private
   */
  ara::core::Result<bool> Verify(
      ara::crypto::ReadOnlyMemRegion value, ara::crypto::ReadOnlyMemRegion signature,
      // VECTOR NL AutosarC++17_10-M8.3.1: MD_CRYPTO_AutosarC++17_10-M8.3.1_namespace
      ara::crypto::ReadOnlyMemRegion context = ara::crypto::ReadOnlyMemRegion{}) const noexcept override;
...
#endif  // LIB_LIBCLIENT_INCLUDE_CRYPTO_CLIENT_PROXY_CRYP_VERIFIER_PUBLIC_CTX_H_
