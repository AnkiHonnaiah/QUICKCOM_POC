 ...
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file
 *        \brief  JSON parser of a 'RequiredServiceInstance' runtime configurations.
 *
 *********************************************************************************************************************/

#ifndef LIB_IPC_BINDING_INCLUDE_IPC_BINDING_INTERNAL_CONFIG_PARSER_REQUIRED_SERVICE_INSTANCES_JSON_PARSER_H_
#define LIB_IPC_BINDING_INCLUDE_IPC_BINDING_INTERNAL_CONFIG_PARSER_REQUIRED_SERVICE_INSTANCES_JSON_PARSER_H_
 ...
 */
class RequiredServiceInstanceJsonParser final : public ::amsr::json::v2::SingleObjectParser {
 public:
  /*! Type name alias for amsr::ipc_binding_core::internal::runtime_configuration::RequiredServiceInstanceConfig. */
  using RequiredServiceInstanceConfig =
      ::amsr::ipc_binding_core::internal::runtime_configuration::RequiredServiceInstanceConfig;

  /*!
   * \brief Constructor for RequiredServiceInstanceJsonParser.
   *
   ...
   * \steady  FALSE
   */
  RequiredServiceInstanceJsonParser(JsonData& json, RequiredServiceInstanceConfig& config) noexcept;
   ...
   * \steady      FALSE
   */
  ~RequiredServiceInstanceJsonParser() noexcept final = default;  // VCA_IPCB_MOLE_1298

  RequiredServiceInstanceJsonParser(RequiredServiceInstanceJsonParser const&) noexcept = delete;
  auto operator=(RequiredServiceInstanceJsonParser const&) noexcept -> RequiredServiceInstanceJsonParser& = delete;
  RequiredServiceInstanceJsonParser(RequiredServiceInstanceJsonParser&&) noexcept = delete;
  auto operator=(RequiredServiceInstanceJsonParser&&) noexcept -> RequiredServiceInstanceJsonParser& = delete;
 ...
 private:
  ...
  static constexpr StringView kMinorVersion{"minor_version"_sv};
  /*! JSON key for expected server integrity level config. */
  static constexpr StringView kExpectedIntegrityLevelKey{"expected_server_integrity_level"_sv};
  /*! JSON key for expected server integrity level QM. */
  static constexpr StringView kExpectedIntegrityLevelQM{"QM"_sv};
  /*! JSON key for expected server integrity level ASIL A. */
  static constexpr StringView kExpectedIntegrityLevelAsilA{"ASIL_A"_sv};
  /*! JSON key for expected server integrity level ASIL B. */
  static constexpr StringView kExpectedIntegrityLevelAsilB{"ASIL_B"_sv};
  /*! JSON key for expected server integrity level ASIL C. */
  static constexpr StringView kExpectedIntegrityLevelAsilC{"ASIL_C"_sv};
  /*! JSON key for expected server integrity level ASIL D. */
  static constexpr StringView kExpectedIntegrityLevelAsilD{"ASIL_D"_sv};

  /*! Callback to parse the value for 'instance_id' key.
  ...
  /*!
   * \brief Callback to parse the array of 'expected_server_integrity_level' key.
   *
   * \return A result to indicate whether parsing should continue.
   *
   * \error IpcBindingErrc::json_parsing_failure  If parsing the value fails.
  ...
  std::size_t number_of_expected_integrity_level_found_{
      0}; /*!< Number of key 'expected_server_integrity_level' found.*/

  /*! Reference to service instance config. */
  RequiredServiceInstanceConfig& config_;
};
...
#endif  // LIB_IPC_BINDING_INCLUDE_IPC_BINDING_INTERNAL_CONFIG_PARSER_REQUIRED_SERVICE_INSTANCES_JSON_PARSER_H_
