 ...
 *********************************************************************************************************************/
/****************************************************************************
 ** Number of attempts to get a non-zero random number.
 ** The random number itself is generated by the callout esl_getBytes,
 ** function actBNModRandomize() then applies post-processing steps to
 ** reduce the raw random number by a given modulus.
 ** If the overall result is zero, a new random number shall be generated.
 ** This process shall at most be repeated VSECPRIM_MOD_RANDOM_MAX_RETRIES
 ** many times.
 ***************************************************************************/
# define actBNMODRANDOMIZE_RNG_MAX_RETRIES                            (VSECPRIM_MOD_RANDOM_MAX_RETRIES)
...
/* Note: The implementation of the algorithms AES, ChaCha20 and SHA provides different speed-up level.
 * Depending on the speed up level, for-loops are unrolled, functions are replaced by function macros
 * and/or look-up tables are used.
 * Run-time/performance depends on many parameters like caching, hardware, compiler/compiler-optimization, etc.
 * This could also lead to larger binary code which affects e.g. caching and negatively impacts run-time/performance.
 * Hence, a higher speed-up level does not directly imply a better run-time/performance. */
