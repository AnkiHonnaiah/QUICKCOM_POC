 ...
 * \param[in,out]  workSpace     (in) algorithm context buffer, initialized by esl_initFIPS186(..) function
 *                               (out) updated algorithm context structure
 * \param[in]      inputLength   the number of input bytes used to stir up the random number generator
 ...
 *  \details       This function calls esl_initialSeed_CTRDRBG() or esl_reseed_CTRDRBG, depending on the seed status.
 *  \param[in,out] workSpace                     (in)  pointer to the DRBG workspace
 ...
 *  \details       This function calls esl_initialSeed_HASHDRBG() or esl_reseed_HASHDRBG, depending on the seed status.
 *  \param[in,out] workSpace                     (in)  pointer to the DRBG workspace
 ...
 * \details        This function finalizes the AES encryption by processing the remaining input bytes in the
 ...
 * \details        This function finalizes the AES decryption by processing the remaining input bytes in the
 *                 internal buffer.
 * \param[in,out]  workSpace   (in) algorithm context structure
 *                             (out) updated algorithm context structure
 * \param[in,out]  outputSize  (in) output buffer size in bytes, a safe length is AES_BLOCK_LENGTH
 *                             (out) total size of decrypted input without padding, at most AES_BLOCK_LENGTH bytes
 ...
 *********************************************************************************************************************/
/*! \brief          This function finalizes the AES encryption by processing the
 ...
 *********************************************************************************************************************/
/*! \brief          This function finalizes the AES decryption by processing the
 ...
 *********************************************************************************************************************/
/*! \brief          This function finalizes the DES encryption by processing the
 *                  remaining input bytes in the internal buffer and padding the
 *                  input according to the chosen padding in the corresponding
 *                  init function.
 *  \details        -
 ...
 *********************************************************************************************************************/
/*! \brief          This function finalizes the DES decryption by processing the
 *                  remaining input bytes in the internal buffer.
 *  \details        -
 *  \param[out]     workSpace    actualized algorithm context structure
 *  \param[in,out]  outputSize   (in) output buffer size in bytes, a safe length is
 *                               DES_BLOCK_LENGTH
 *                               (out) total size of decrypted input
 *                               8 or 16 bytes
 *  \param[in,out]  output       (in) pointer to decrypted data buffer
 *                               (out) pointer to encrypted data blocks
 *  \return         ESL_ERC_PARAMETER_INVALID input parameter is NULL
 ...
 *********************************************************************************************************************/
/*! \brief          This function finalizes the TDES encryption by processing the
 *                  remaining input bytes in the internal buffer and padding the
 *                  input according to the chosen padding in the corresponding
 *                  init function.
 *  \details        -
 ...
 *********************************************************************************************************************/
/*! \brief          This function finalizes the TDES decryption by processing the
 *                  remaining input bytes in the internal buffer.
 *  \details        -
 *  \param[out]     workSpace    actualized algorithm context structure
 *  \param[in,out]  outputSize   (in) output buffer size in bytes, a safe length is
 ...
 *********************************************************************************************************************/
/*! \brief          This function finalizes the RC2 encryption by processing the
 *                  remaining input bytes in the internal buffer and padding the
 *                  input according to the chosen padding in the corresponding
 *                  init function.
 *  \details        -
 ...
 *********************************************************************************************************************/
/*! \brief          This function finalizes the RC2 decryption by processing the
 *                  remaining input bytes in the internal buffer.
 *  \details        -
 *  \param[out]     workSpace    actualized algorithm context structure
 *  \param[in,out]  outputSize   (in) output buffer size in bytes, a safe length is
 ...
 *                               esl_initHashMACSHA384(..) function
 *                               (out) actualized algorithm context structure
 *  \param[in,out]  messageHashMAC  (in) pointer to buffer for HashMAC (at least 48 bytes)
 *                               (out) pointer to computed HashMAC
 *  \return         ESL_ERC_PARAMETER_INVALID input parameter is NULL
 *                  ESL_ERC_WS_STATE_INVALID  work space state invalid
 *                  ESL_ERC_WS_TOO_SMALL      work space too small
 ...
 *                               esl_initHashMACSHA384(..) function
 *                               (out) actualized algorithm context structure
 *  \param[in]      messageHashMAC  pointer to the given HashMAC (48 bytes)
 *  \return         ESL_ERC_PARAMETER_INVALID input parameter is NULL
 *                  ESL_ERC_WS_STATE_INVALID  work space state invalid
 *                  ESL_ERC_WS_TOO_SMALL      work space too small
 *                  ESL_ERC_HMAC_INCORRECT_MAC   the given HashMAC is incorrect
...
# if (VSECPRIM_HMAC_SHA2_512_ENABLED == STD_ON) /* COV_VSECPRIM_NO_SAFE_CONFIG XF */
/*****************************************************************************
 * HashMACSHA512 crypto functions
 *****************************************************************************/
/**********************************************************************************************************************
 *  esl_generateKeyHashMACSHA512()
 *********************************************************************************************************************/
/*! \brief          This function generates a HMAC key for an HashMac calculation. It uses the RNG which is
 *                  implemented by the call back function esl_getBytesRNG(..).
 *  \details        -
 ...
 *  \synchronous    TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_generateKeyHashMACSHA512(
  const eslt_Length keyLength, VSECPRIM_P2VAR_PARA(eslt_Byte) key);

/**********************************************************************************************************************
 *  esl_initHashMACSHA512()
 *********************************************************************************************************************/
/*! \brief          This function initializes the HashMAC calculation according
 *                  to RFC2104 based on the hash function SHA-512.
 *  \details        -
 *  \param[in,out]  workSpace    (in) algorithm context buffer, initialized by
 *                               esl_initWorkSpaceHeader(..) function
 *                               (out) initialized algorithm context structure
 ...
 *  \synchronous    TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_initHashMACSHA512(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceHMACSHA512) workSpace,
  const eslt_Length keyLength, VSECPRIM_P2CONST_PARA(eslt_Byte) key);

/**********************************************************************************************************************
 *  esl_updateHashMACSHA512()
 *********************************************************************************************************************/
/*! \brief          This function updates the HashMAC calculation according
 *                  to RFC2104 based on the hash function SHA-512.
 *  \details        -
 *  \param[in,out]  workSpace    (in) algorithm context buffer, initialized by
 *                               esl_initHashMACSHA512(..) function
 *                               (out) actualized algorithm context structure
 *  \param[in]      inputLength  length of input data in bytes
 *  \param[in]      input        pointer to data to be handled
 *  \return         ESL_ERC_PARAMETER_INVALID input parameter is NULL
 *                  ESL_ERC_WS_STATE_INVALID  work space state invalid
 *                  ESL_ERC_WS_TOO_SMALL      work space too small
 *                  ESL_ERC_TOTAL_LENGTH_OVERFLOW   total input length overflow
 *                  ESL_ERC_NO_ERROR          else
 *  \pre            workSpace is initialized by esl_initHashMACSHA512(..)
 *  \context        TASK
 *  \reentrant      TRUE, for different workspaces
 *  \synchronous    TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_updateHashMACSHA512(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceHMACSHA512) workSpace,
  const eslt_Length inputLength, VSECPRIM_P2CONST_PARA(eslt_Byte) input);

/**********************************************************************************************************************
 *  esl_finalizeHashMACSHA512()
 *********************************************************************************************************************/
/*! \brief          This function finalizes the HashMAC calculation by returning
 *                  the calculated HashMAC according to RFC2104 based on SHA-512.
 *  \details        -
 *  \param[in,out]  workSpace    (in) algorithm context buffer, initialized by
 *                               esl_initHashMACSHA512(..) function
 *                               (out) actualized algorithm context structure
 *  \param[in,out]  messageHashMAC  (in) pointer to buffer for HashMAC (at least 64 bytes)
 *                               (out) pointer to computed HashMAC
 *  \param[in,out]  messageHashMACLength   (in) pointer to messageHashMACLength >= 64 bytes
 *                                      (out) The written messageHashMAC length is set to ESL_SIZEOF_SHA512_DIGEST
 *  \return         ESL_ERC_PARAMETER_INVALID input parameter is NULL
 *                  ESL_ERC_INPUT_INVALID     messageHashMACLength is smaller than correct size
 *                  ESL_ERC_WS_STATE_INVALID  work space state invalid
 *                  ESL_ERC_WS_TOO_SMALL      work space too small
 *                  ESL_ERC_NO_ERROR          success
 *  \pre            workSpace is initialized by esl_initHashMACSHA512(..)
 *                  esl_updateHashMACSHA512 called before
 *  \context        TASK
 *  \reentrant      TRUE, for different workspaces
 *  \synchronous    TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_finalizeHashMACSHA512(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceHMACSHA512) workSpace, VSECPRIM_P2VAR_PARA(eslt_Byte) messageHashMAC,
  VSECPRIM_P2VAR_PARA(eslt_Length) messageHashMACLength);

/**********************************************************************************************************************
 *  esl_verifyHashMACSHA512()
 *********************************************************************************************************************/
/*! \brief          This function finalizes the verification of a given HashMAC
 *                  according to RFC2104 based on SHA-512.
 *  \details        -
 *  \param[in,out]  workSpace    (in) algorithm context buffer, initialized by
 *                               esl_initHashMACSHA512(..) function
 *                               (out) actualized algorithm context structure
 *  \param[in]      messageHashMAC  pointer to the given HashMAC (64 bytes)  
 *  \param[in]      messageHashMACLength  the byte length of messageHashMac
 *  \return         ESL_ERC_PARAMETER_INVALID input parameter is NULL
 *                  ESL_ERC_INPUT_INVALID     messageHashMACLength is not of correct size
 *                  ESL_ERC_WS_STATE_INVALID  work space state invalid
 *                  ESL_ERC_WS_TOO_SMALL      work space too small
 *                  ESL_ERC_HMAC_INCORRECT_MAC   the given HashMAC is incorrect
 *                  ESL_ERC_NO_ERROR          HashMAC is correct
 ...
 *  \synchronous    TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_verifyHashMACSHA512(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceHMACSHA512) workSpace, VSECPRIM_P2CONST_PARA(eslt_Byte) messageHashMAC,
  const eslt_Length messageHashMACLength);
# endif /* (VSECPRIM_HMAC_SHA2_512_ENABLED == STD_ON) */
...
# if ((VSECPRIM_HKDF_SHA2_256_ONESTEP_ENABLED == STD_ON) || (VSECPRIM_HKDF_SHA2_512_ONESTEP_ENABLED == STD_ON))
/***********************************************************************************************************************
 ...
 **********************************************************************************************************************/
  ...
  *                 Currently, only SHA2-256 and SHA2-512 are supported.
  ...
  *                            and ESL_HA_SHA2_512 are accepted.
  *  \return        ESL_ERC_NO_ERROR           Operation successful
  *                 ESL_ERC_WS_TOO_SMALL       Work space too small
  *                 ESL_ERC_PARAMETER_INVALID  Input parameter is NULL
  *                 ESL_ERC_INPUT_INVALID      hashID != ESL_HA_SHA2_256 and hashID != ESL_HA_SHA2_512
  *  \pre           workSpace is initialized by esl_initWorkSpaceHeader(..)
  *  \context       TASK
  *  \reentrant     TRUE, for different workspaces.
  *  \synchronous   TRUE
 ...
 **********************************************************************************************************************/
  ...
  *                 ESL_ERC_WS_TOO_SMALL       Work space too small
  *                 ESL_ERC_WS_STATE_INVALID   Work space state invalid
  *                 ESL_ERC_INVALID_LENGTH     Invalid input length, i.e., keyLength > 4.294.967.264 bytes for SHA2-256
  *                                            and keyLength > 4.294.967.232 bytes for SHA2-512.
...
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_deriveKeyHKDF_Hash_OneStep(
  ...
  const eslt_Length keyLength,
  VSECPRIM_P2VAR_PARA(eslt_Byte) key);
# endif /* ((VSECPRIM_HKDF_SHA2_256_ONESTEP_ENABLED == STD_ON) || (VSECPRIM_HKDF_SHA2_512_ONESTEP_ENABLED == STD_ON)) */
...
/***********************************************************************************************************************
 *  esl_expandKeyHKDF_HMAC_SHA256
 **********************************************************************************************************************/
/*! \brief         Derive Key according to HKDF HMAC with SHA256 using a given pseudo random key
 *  \details       This function derives a key with the two-step method (extraction and expansion) according
 *                 to HKDF HMAC with SHA256 with the difference, that the extraction step from the two-step method is 
 *                 omitted. The key that is normally generated within this function needs to be given.
 *                 This is the so called pseudo random key.
 *                 With the pseudo random key the expansion is performed.
 *                 The method is determined by the mode flag, a parameter for esl_initHKDF_HMAC_SHA256,
 *                 which is stored in the workspace.
 *  \param[in,out] workSpace      (in)  HKDF HMAC with SHA256 work space
 *                                (out) HKDF HMAC with SHA256 work space
 *  \param[in]     keyDerivationKey   pointer to given pseudo random key
 *  \param[in]     keyDerivationKeyLength         length of the pseudo random key (32 byte)
 *  \param[in]     info           pointer to the optional additional input, can be NULL
 *  \param[in]     infoLength     length of additional input in bytes, can be zero
 *  \param[out]    key            pointer to the output key buffer
 *  \param[in]     keyLength      desired length of the key to be derived in bytes
 *  \return        ESL_ERC_NO_ERROR           Operation successful
 *                 ESL_ERC_PARAMETER_INVALID  Input parameter is NULL
 *                 ESL_ERC_INPUT_INVALID      keyLength = 0 or keyDerivationKeyLength = 0 or keyDerivationKeyLength != 32
 *                 ESL_ERC_WS_TOO_SMALL       Work space too small
 *                 ESL_ERC_WS_STATE_INVALID   Work space state invalid
 *                 ESL_ERC_INVALID_LENGTH     Invalid input length, i.e.,
 *                                            keyLength > 255*SHA256 digest length (in bytes) for the two step method
 ...
 *  \synchronous   TRUE
 **********************************************************************************************************************/
 VSECPRIM_FUNC(eslt_ErrorCode) esl_expandKeyHKDF_HMAC_SHA256(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceHKDFHMAC) workSpace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) keyDerivationKey,
  const eslt_Length keyDerivationKeyLength,
  VSECPRIM_P2CONST_PARA(eslt_Byte) info,
  const eslt_Length infoLength,
  VSECPRIM_P2VAR_PARA(eslt_Byte) key,
  const eslt_Length keyLength);
...
/***********************************************************************************************************************
 *  esl_initHKDF_HMAC_SHA384
 **********************************************************************************************************************/
/*! \brief         Initializes HKDF HMAC with SHA384 calculation.
 *  \details       This function initializes one step (Option 2) or two step HKDF HMAC with SHA384 calculation.
 *  \param[in,out] workSpace  (in)  HKDF HMAC SHA384 work space
 *                            (out) HKDF HMAC SHA384 work space
 *  \param[in]     mode       HKDF mode, indicating whether the one-step or the two-step variant shall be used
 *  \return        ESL_ERC_NO_ERROR           Operation successful
 *                 ESL_ERC_WS_TOO_SMALL       Work space too small
 *                 ESL_ERC_PARAMETER_INVALID  Input parameter is NULL
 ...
 *  \synchronous   TRUE
 **********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_initHKDF_HMAC_SHA384(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceHKDFHMAC) workSpace,
  eslt_HKDFMode mode);

/***********************************************************************************************************************
 *  esl_deriveKeyHKDF_HMAC_SHA384
 **********************************************************************************************************************/
/*! \brief         Derive Key according to HKDF HMAC with SHA384
 *  \details       This function derives a key according to HKDF HMAC with SHA384.
 *                 Either the one-step method (Option 2) or the two-step method (extraction and expansion step)
 *                 is performed. The method is determined by the mode flag, a parameter for esl_initHKDF_HMAC_SHA384,
 *                 which is stored in the workspace.
 *  \param[in,out] workSpace      (in)  HKDF HMAC with SHA384 work space
 *                                (out) HKDF HMAC with SHA384 work space
 *  \param[in]     secretLength   length of secret in bytes
 *  \param[in]     secret         Pointer to the secret
 *  \param[in]     saltLength     length of salt in bytes (if zero, an all zero vector is used instead)
 *  \param[in]     salt           pointer to the optional salt (if NULL, an all zero vector is used instead)
 ...
 *                 ESL_ERC_WS_STATE_INVALID   Work space state invalid
 *                 ESL_ERC_INVALID_LENGTH     Invalid input length, i.e., 
 *                                            keyLength > 4 294 967 248 bytes for the one step method, or
 *                                            keyLength > 255*SHA384 digest length (in bytes) for the two step method
 *  \pre           workSpace is initialized by esl_initHKDF_HMAC_SHA384(..)
 *                 The buffer referenced by key needs to provide at least keyLength many bytes.
 *  \context       TASK
 *  \reentrant     TRUE, for different workspaces.
 *  \synchronous   TRUE
 **********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_deriveKeyHKDF_HMAC_SHA384(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceHKDFHMAC) workSpace,
  const eslt_Length secretLength,
  VSECPRIM_P2CONST_PARA(eslt_Byte) secret,
  const eslt_Length saltLength,
...
/***********************************************************************************************************************
 *  esl_expanedKeyHKDF_HMAC_SHA384
 **********************************************************************************************************************/
/*! \brief         Derive Key according to HKDF HMAC with SHA384 using a given pseudo random key
 *  \details       This function derives a key with the two-step method (extraction and expansion) according
 *                 to HKDF HMAC with SHA384 with the difference, that the extraction step from the two-step method is 
 *                 omitted. The key that is normally generated within this function needs to be given.
 *                 This is the so called pseudo random key.
 *                 With the pseudo random key the expansion is performed.
 *                 The method is determined by the mode flag, a parameter for esl_initHKDF_HMAC_SHA256,
 ...
 *                                (out) HKDF HMAC with SHA384 work space
 *  \param[in]     keyDerivationKey   pointer to given pseudo random key
 *  \param[in]     keyDerivationKeyLength         length of the pseudo random key (48 byte) 
 *  \param[in]     info           pointer to the optional additional input, can be NULL
 *  \param[in]     infoLength     length of additional input in bytes, can be zero
 *  \param[out]    key            pointer to the output key buffer
 *  \param[in]     keyLength      desired length of the key to be derived in bytes
 *  \return        ESL_ERC_NO_ERROR           Operation successful
 *                 ESL_ERC_PARAMETER_INVALID  Input parameter is NULL
 *                 ESL_ERC_INPUT_INVALID      keyLength = 0 or keyDerivationKeyLength = 0 or keyDerivationKeyLength != 48
 *                 ESL_ERC_WS_TOO_SMALL       Work space too small
 *                 ESL_ERC_WS_STATE_INVALID   Work space state invalid
 *                 ESL_ERC_INVALID_LENGTH     Invalid input length, i.e.,
 *                                            keyLength > 255*SHA384 digest length (in bytes) for the two step method
 ...
 *  \synchronous   TRUE
 **********************************************************************************************************************/
 extern VSECPRIM_FUNC(eslt_ErrorCode) esl_expandKeyHKDF_HMAC_SHA384(VSECPRIM_P2VAR_PARA(eslt_WorkSpaceHKDFHMAC) workSpace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) keyDerivationKey,
  const eslt_Length keyDerivationKeyLength,
  VSECPRIM_P2CONST_PARA(eslt_Byte) info,
  const eslt_Length infoLength,
 ...
 *  \param[in]      speedUpExt   pointer to pre computation structure
 *  \return         ESL_ERC_PARAMETER_INVALID input parameter is NULL
 *                  ESL_ERC_WS_TOO_SMALL      work space too small
 ...
 *****************************************************************************/
/**********************************************************************************************************************
 *  esl_initKeyCheckEcP_prim()
 *********************************************************************************************************************/
/*! \brief          This function initializes the EcP key validity check.
 *  \details        -
 *  \param[in,out]  workspace  workspace including the underlying curve
 *  \param[in]      domain       pointer to domain parameter structure
 *  \param[in]      domainExt    pointer to domain parameter extension structure
 ...
 *               speedUpExt has to be a valid ASN1 struct for precomputed EC points
 *               The addresses of domain, domainExt and speedUpExt have to be valid until returning from
 *               esl_publicKeyCheckEcP_prim() or esl_privateKeyCheckEcP_prim(), because the pointers to the data 
 *               stored in domain, domainExt and speedUpExt are stored in the workspace. 
 *               These pointers are used as long as the workspace is used.
 *  \context     TASK
 *  \reentrant   TRUE, for different workspaces
 *  \synchronous TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_initKeyCheckEcP_prim(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEcPKeyCheck) workspace,
  VSECPRIM_P2ROMCONST_PARA(eslt_EccDomain) domain,
  VSECPRIM_P2ROMCONST_PARA(eslt_EccDomainExt) domainExt);

/**********************************************************************************************************************
 *  esl_publicKeyCheckEcP_prim()
 *********************************************************************************************************************/
/*! \brief        This function checks if a given public key is valid.
 *  \details      -
 *  \param[in,out]  workspace  workspace including the underlying curve
 *  \param[in]      publicKey_x        pointer to the given x-coordinate of the public key
 *  \param[in]      publicKeyLength_x  length of the public key's x-coordinate
 *  \param[in]      publicKey_y        pointer to the given y-coordinate of the public key
 ...
 *               ESL_ERC_INPUT_INVALID          - the given length is not as expected
 *               ESL_ERC_KEY_INVALID            - the given public key is invalid
 *  \pre         workspace is initialized by esl_initKeyCheckEcP_prim(..) 
 *  \context     TASK
 *  \reentrant   TRUE, for different workspaces
 *  \synchronous TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_publicKeyCheckEcP_prim(VSECPRIM_P2CONST_PARA(eslt_WorkSpaceEcPKeyCheck) workspace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) publicKey_x, const eslt_Length publicKeyLength_x,
  VSECPRIM_P2CONST_PARA(eslt_Byte) publicKey_y, const eslt_Length publicKeyLength_y);

/**********************************************************************************************************************
 *  esl_privateKeyCheckEcP_prim()
 *********************************************************************************************************************/
/*! \brief        This function checks if a given private key is valid.
 *  \details      -
 *  \param[in,out]  workspace         workspace including the underlying curve
 ...
 *               ESL_ERC_WS_STATE_INVALID       - wrong workspace state
 *               ESL_ERC_INPUT_INVALID          - the given length is not as expected
 *               ESL_ERC_KEY_INVALID            - the given private key is invalid
 *  \pre         workspace is initialized by esl_initKeyCheckEcP_prim(..)
 *  \context     TASK
 *  \reentrant   TRUE, for different workspaces
 *  \synchronous TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_privateKeyCheckEcP_prim(
  VSECPRIM_P2CONST_PARA(eslt_WorkSpaceEcPKeyCheck) workspace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) privateKey, 
  const eslt_Length privateKeyLength);
 ...
 *  esl_signEdDSA()
 *********************************************************************************************************************/
/*! \brief       Deprecated! Sign data EdDSA.
 ...
 *               ESL_ERC_ERROR                 - unknown exception
 *  \pre         workSpace is initialized by esl_initWorkSpaceHeader(..)
 *  \context     TASK
 *  \reentrant   TRUE, for different workspaces
 *  \synchronous TRUE
 ...
 *  esl_initVerifyEdDSA()
 *********************************************************************************************************************/
/*! \brief       Deprecated! Init verification for EdDSA.
 ...
 *  esl_verifyEdDSA()
 *********************************************************************************************************************/
/*! \brief       Deprecated! EdDSA verification.
 ...
 *  esl_updateEdDSA()
 *********************************************************************************************************************/
/*! \brief       Deprecated! Perform hashing
 ...
 *               ESL_ERC_WS_STATE_INVALID      - work space state invalid
 *               ESL_ERC_WS_TOO_SMALL          - work space too small
 *               ESL_ERC_INPUT_INVALID          algorithm instance (stored in workspace) is not 'prehash'
 *               ESL_ERC_TOTAL_LENGTH_OVERFLOW - internal overflow (== message too long - SHA-512)
 *  \pre         workSpace is initialized by esl_initWorkSpaceHeader(..)
 *  \context     TASK
 *  \reentrant   TRUE, for different workspaces
...
# if (VSECPRIM_EDDSA_ENABLED == STD_ON)
/**********************************************************************************************************************
 *  esl_initKeyCheckEdwards()
 **********************************************************************************************************************/
/*! \brief         Initialize Edwardscurve key check
 *  \details       This function initializes the Ed25519 or Ed448 key check.
 ...
 *  \synchronous   TRUE
 *********************************************************************************************************************/
VSECPRIM_FUNC(eslt_ErrorCode) esl_initKeyCheckEdwards(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdKeyCheck) workspace,
  const eslt_Size16 curve);

/**********************************************************************************************************************
 * esl_publicKeyCheckEdwards()
 **********************************************************************************************************************/
/*! \brief         This function checks if a public key is valid by checking if it is on the curve, within range and
 *                 of expected length.
 *  \details       This function checks if a given public key for Ed25519 or Ed448 is valid, by decoding the public 
 *                 key. During the decoding it's checked that it must be in range [0,p) and it must lie on the curve.
 *                 According to NIST Special Publication NIST SP 800-186 "Recommendations for Discrete Logarithm-based 
 *                 Cryptography: Elliptic Curve Domain Parameters" of February 2023 this is only a partial public key
 *                 check. Furthermore, it's checked, that the public key is of the expected length. 
 *                 The expected key length is 32 bytes for Ed25519 or 57 bytes for Ed448 respectively.
 *  \param[in,out] workspace       Pointer to initialized workspace
 ...
 *                 ESL_ERC_WS_TOO_SMALL         Workspace too small
 *                 ESL_ERC_WS_STATE_INVALID     Workspace state invalid
 *                 ESL_ERC_INPUT_INVALID        The length of the public key is not as expected
 *                 ESL_ERC_KEY_INVALID          Public key is invalid
 *  \pre           Workspace needs to be initialized. Pointers must be valid.
 *  \context       TASK
 *  \reentrant     TRUE, for different workspaces
 *  \synchronous   TRUE
 *********************************************************************************************************************/
VSECPRIM_FUNC(eslt_ErrorCode) esl_publicKeyCheckEdwards(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdKeyCheck) workspace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) publicKey,
  const eslt_Length publicKeyLength);

/**********************************************************************************************************************
 *  esl_privateKeyCheckEdwards()
 **********************************************************************************************************************/
/*! \brief         This function checks if a private key is valid.
 *  \details       This function checks if a given private key for Ed25519 or Ed448 is valid. Therefore, it must 
 *                 neither be zero, nor unequal to the expected key length. The expected key length is 
 *                 32 bytes for Ed25519 respectively 57 bytes for Ed448.
 *  \param[in]     workspace        Pointer to initialized workspace
 *  \param[in]     privateKey       Pointer to private key
 ...
 *                 ESL_ERC_WS_TOO_SMALL         Workspace too small
 *                 ESL_ERC_WS_STATE_INVALID     Workspace state invalid
 *                 ESL_ERC_INPUT_INVALID        The private key length is not as expected
 *                 ESL_ERC_KEY_INVALID          Private key is invalid
 *  \pre           Workspace needs to be initialized. Pointers must be valid.
 *  \context       TASK
 *  \reentrant     TRUE, for different workspaces
 *  \synchronous   TRUE
 *********************************************************************************************************************/
VSECPRIM_FUNC(eslt_ErrorCode) esl_privateKeyCheckEdwards(
  VSECPRIM_P2CONST_PARA(eslt_WorkSpaceEdKeyCheck) workspace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) privateKey,
  const eslt_Length privateKeyLength);

/**********************************************************************************************************************
 *  esl_initSignEdwardsDSA()
 **********************************************************************************************************************/
/*! \brief         Initialize EdDSA signature generation.
 *  \details       This function initializes the Ed25519 or the Ed448 algorithm for signature generation,
 *                 depending on the value of 'curve'.
 *  \param[in,out] workSpace         Pointer to initialized workspace buffer
 *  \param[in]     curve             underlying curve; supports
 *                                   - ESL_CURVE25519
 *                                   - ESL_CURVE448
 ...
 *  \return        ESL_ERC_NO_ERROR               Operation successful
 *                 ESL_ERC_PARAMETER_INVALID      Input parameter is NULL or context length too high
 *                 ESL_ERC_WS_TOO_SMALL           Work space too small
 *                 ESL_ERC_CURVE_NOT_SUPPORTED    Requested curve is not supported
 *                 ESL_ERC_INSTANCE_NOT_SUPPORTED Requested instance not supported
 *  \pre           workSpace is initialized by esl_initWorkSpaceHeader(..)
 *  \context       TASK
 *  \reentrant     TRUE, for different workspaces
 *  \synchronous   TRUE
 *********************************************************************************************************************/
VSECPRIM_FUNC(eslt_ErrorCode) esl_initSignEdwardsDSA(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdDSA) workSpace,
  eslt_Size16 curve,
  const eslt_Byte instance,
  VSECPRIM_P2CONST_PARA(eslt_Byte) context,
  const eslt_Length contextLength);
 ...
 *  esl_signEdwardsDSA()
 *********************************************************************************************************************/
/*! \brief          Sign data with EdDSA.
 *  \details        This function signs the given input data using Ed25519 or Ed448.
 *                  for Ed25519: use SHA512 with dom2 for hashing, key length is 32 byte for public/private keys,
 *                               signature length is 64 byte
 *                  for Ed448: use SHAKE256 with dom4 for hashing, key length is 57 byte for public/private keys,
 *                             signature length is 114 byte
 *  \param[in,out]  workSpace        Algorithm context buffer, initialized by esl_initSignEdwardsDSA(..) function
 *  \param[in]      input            Pointer to the input data
 *  \param[in]      inputLength      The length of the provided input data
 *  \param[in]      secretKey        Pointer to the secret key of length 32 byte for Ed25519 or 57 byte for Ed448 to use for signature
 *  \param[in]      secretKeyLength  The length of the secretKey (32 byte/57 byte)
 *  \param[in]      publicKey        Pointer to the public key of length 32 byte/ 57 byte. Pass null pointer to calculate from private key.
 *  \param[in]      publicKeyLength  The length of the publicKey (32 byte/57 byte)
 *  \param[out]     signature        Pointer to where the signature shall be stored.
 *  \param[in,out]  signatureLength  Pointer to where the size of the buffer is provided
 *  \return         ESL_ERC_NO_ERROR               Operation successful
 *                  ESL_ERC_PARAMETER_INVALID      Some parameter is NULL
 *                  ESL_ERC_WS_STATE_INVALID       Work space state invalid
 *                  ESL_ERC_WS_TOO_SMALL           Work space too small
 *                  ESL_ERC_INVALID_LENGTH         Input length invalid
 *                  ESL_ERC_BUFFER_TOO_SMALL       Buffer for signature too small
 *                  ESL_ERC_TOTAL_LENGTH_OVERFLOW  Internal overflow (== message too long for SHA-512,
 *                                                 wrong phase for SHAKE256)
 *                  ESL_ERC_ERROR                  Unknown exception
 *  \pre            workSpace is initialized by esl_initSignEdwardsDSA(..)
 *  \context        TASK
 *  \reentrant      TRUE, for different workspaces
 *  \synchronous    TRUE
 *********************************************************************************************************************/
VSECPRIM_FUNC(eslt_ErrorCode) esl_signEdwardsDSA(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdDSA) workSpace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) input,
  const eslt_Length inputLength,
  VSECPRIM_P2CONST_PARA(eslt_Byte) secretKey,
  const eslt_Length secretKeyLength,
  VSECPRIM_P2CONST_PARA(eslt_Byte) publicKey,
  const eslt_Length publicKeyLength,
  VSECPRIM_P2VAR_PARA(eslt_Byte) signature,
  VSECPRIM_P2VAR_PARA(eslt_Length) signatureLength);
...
/**********************************************************************************************************************
 *  esl_initVerifyEdwardsDSA()
 *********************************************************************************************************************/
/*! \brief          Initialize EdDSA signature verification.
 *  \details        This function initializes the Ed25519 or the Ed448 algorithm for signature verification,
 *                  depending on the value of 'curve'.
 *  \param[in,out]  workSpace      algorithm context buffer, initialized by esl_initWorkSpaceHeader(..) function
 *  \param[in]      curve          Underlying curve; supports
 *                                 - ESL_CURVE25519
 ...
 *  \synchronous TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_initVerifyEdwardsDSA(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdDSA) workSpace,
  eslt_Size16 curve,
  const eslt_Byte instance,
  VSECPRIM_P2CONST_PARA(eslt_Byte) context,
...
/**********************************************************************************************************************
 *  esl_verifyEdwardsDSA()
 *********************************************************************************************************************/
/*! \brief         Verify signature with EdDSA.
 *  \details       This function verifies given signature against the given data using Ed25519 or Ed448.
 *                 for Ed25519: use SHA512 with dom2 for hashing, key length is 32 byte for public keys,
 *                              signature length is 64 byte
 *                 for Ed448: use SHAKE256 with dom4 for hashing, key length is 57 byte for public,
 *                            signature length is 114 byte
 *  \param[in,out]  workSpace        Algorithm context buffer, initialized by esl_initVerifyEdwardsDSA(..) function
 *  \param[in]      input            Pointer to the input data.
 *  \param[in]      inputLength      The length of the input data
 *  \param[in]      publicKey        Pointer to the public key
 *  \param[in]      publicKeyLength  Length of the publicKey
 *  \param[in]      signature        Pointer to the signature to be validated against input data
 *  \param[in]      signatureLength  Length of the signature
 *  \return         ESL_ERC_NO_ERROR               Operation successful
 *                  ESL_ERC_PARAMETER_INVALID      Some parameter is NULL
 ...
 *                  ESL_ERC_TOTAL_LENGTH_OVERFLOW  Internal overflow (== message too long for SHA-512,
 *                                                 wrong phase for SHAKE256)
 *                  ESL_ERC_SIGNATURE_INVALID      The signature is NOT valid
 *                  ESL_ERC_INVALID_LENGTH         The public key length and/or the signature length is not as expected
 *  \pre         workSpace is initialized by esl_initVerifyEdwardsDSA(..)
 *  \context     TASK
 *  \reentrant   TRUE, for different workspaces
 *  \synchronous TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) esl_verifyEdwardsDSA(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdDSA) workSpace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) input,
  const eslt_Length inputLength,
  VSECPRIM_P2CONST_PARA(eslt_Byte) publicKey,
  const eslt_Length publicKeyLength,
  VSECPRIM_P2CONST_PARA(eslt_Byte) signature,
  const eslt_Length signatureLength);

/**********************************************************************************************************************
 *  esl_updateEdwardsDSA()
 *********************************************************************************************************************/
/*! \brief          Perform hashing
 *  \details        This function updates the internal hash value in a pre-hash EdDSA25519 or Ed448
 ...
 *                  ESL_ERC_INPUT_INVALID          algorithm instance (stored in workspace) is not 'prehash'
 *                  ESL_ERC_TOTAL_LENGTH_OVERFLOW  Internal overflow (== message too long for SHA-512)
 *                  ESL_ERC_STATE_INVALID          Shake failed
 *  \pre         workSpace is initialized by esl_initWorkSpaceHeader(..)
 *  \context     TASK
 *  \reentrant   TRUE, for different workspaces
 *  \synchronous TRUE
 *********************************************************************************************************************/
VSECPRIM_FUNC(eslt_ErrorCode) esl_updateEdwardsDSA(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdDSA) workSpace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) input,
  const eslt_Length inputLength);
# endif /* (VSECPRIM_EDDSA_ENABLED == STD_ON) */
 ...
 *********************************************************************************************************************/
/*! \brief       Deprecated! Generate Ephemeral Key Pair.
 ...
 *               ESL_ERC_BUFFER_TOO_SMALL    - buffer for private or public key too small
 *               ESL_ERC_RNG_FAILED          - the random number generator failed
 *               ESL_ERC_UNRECOVERABLE_ERROR - invert 0
 *  \pre         workSpace is initialized by esl_initWorkSpaceHeader(..)
 *  \context     TASK
 *  \reentrant   TRUE, for different workspaces
 *  \synchronous TRUE
 ...
 *********************************************************************************************************************/
/*! \brief       Deprecated! Import private key
 ...
 *********************************************************************************************************************/
/*! \brief       Deprecated! Generate shared secret
...
# if (VSECPRIM_EDDH_ENABLED == STD_ON)
/**********************************************************************************************************************
 *  esl_initEdDH()
 *********************************************************************************************************************/
/*! \brief          Init EdDH
 *  \details        This function initializes the Ed-D/H key-exchange.
 *  \param[in,out]  workSpace  algorithm context buffer, initialized by esl_initWorkSpaceHeader(..) function
 *  \param[in]      curve      underlying curve; supports:
 *                                               - ESL_CURVE448
 *                                               - ESL_CURVE25519
 ...
 *  \synchronous    TRUE
 *********************************************************************************************************************/
VSECPRIM_FUNC(eslt_ErrorCode) esl_initEdDH(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdDH) workSpace,
  eslt_Size16 curve);

/**********************************************************************************************************************
 *  esl_generateEphemeralKeyPairEdDH()
 *********************************************************************************************************************/
/*! \brief          Generate Ephemeral Key Pair.
 *  \details        This function generates an ephemeral key-pair for Ed-D/H.
 *  \param[in,out]  workSpace  Algorithm context buffer, initialized by esl_initWorkSpaceHeader(..) function
 *  \param[in,out]  publicKey  Pointer to public key memory
 *  \param[in,out]  publicKeyLength  Pointer to public key memory size
 *  \param[in,out]  privateKey  Pointer to private key memory
 *  \param[in,out]  privateKeyLength  Pointer to private key memory size
 *  \return         ESL_ERC_NO_ERROR - success
 *                  ESL_ERC_PARAMETER_INVALID   - some parameter is NULL
 *                  ESL_ERC_WS_STATE_INVALID    - work space state invalid
 *                  ESL_ERC_WS_TOO_SMALL        - work space too small
 *                  ESL_ERC_BUFFER_TOO_SMALL    - buffer for private or public key too small
 *                  ESL_ERC_RNG_FAILED          - the random number generator failed
 *                  ESL_ERC_UNRECOVERABLE_ERROR - unrecoverable error or invert 0
 *  \pre         workSpace is initialized by esl_initWorkSpaceHeader(..)
 *  \context     TASK
 *  \reentrant   TRUE, for different workspaces
 *  \synchronous TRUE
 *********************************************************************************************************************/
VSECPRIM_FUNC(eslt_ErrorCode) esl_generateEphemeralKeyPairEdDH(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdDH) workSpace,
  VSECPRIM_P2VAR_PARA(eslt_Byte) publicKey,
  VSECPRIM_P2VAR_PARA(eslt_Length) publicKeyLength,
  VSECPRIM_P2VAR_PARA(eslt_Byte) privateKey,
  VSECPRIM_P2VAR_PARA(eslt_Length) privateKeyLength);

/**********************************************************************************************************************
 *  esl_importStaticPrivateKeyEdDH()
 *********************************************************************************************************************/
/*! \brief       Import private key
 *  \details     This function imports a static private key for Ed-D/H.
 *  \param[in,out]  workSpace  Algorithm context buffer, initialized by esl_initWorkSpaceHeader(..) function
 *  \param[in]   privateKey  Pointer to private key
 ...
 *               ESL_ERC_WS_STATE_INVALID     - work space state invalid
 *               ESL_ERC_WS_TOO_SMALL         - work space too small
 *               ESL_ERC_INVALID_LENGTH       - size of private key is too small
 *  \pre         workSpace is initialized by esl_initWorkSpaceHeader(..)
 *  \context     TASK
 *  \reentrant   TRUE, for different workspaces
 *  \synchronous TRUE
 *********************************************************************************************************************/
VSECPRIM_FUNC(eslt_ErrorCode) esl_importStaticPrivateKeyEdDH(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdDH) workSpace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) privateKey,
  const eslt_Length privateKeyLength);

/**********************************************************************************************************************
 *  esl_generateSharedSecretEdDH()
 *********************************************************************************************************************/
 ...
 *  \param[in,out]  sharedSecret  Pointer to shared secret memory
 *  \param[in,out]  sharedSecretLength  Pointer to shared secret memory size
 *  \return         ESL_ERC_NO_ERROR            - success
 *                  ESL_ERC_PARAMETER_INVALID   - some parameter is NULL
 *                  ESL_ERC_WS_STATE_INVALID    - work space state invalid
 ...
 *                  ESL_ERC_BUFFER_TOO_SMALL    - buffer for shared secret too small
 *                  ESL_ERC_UNRECOVERABLE_ERROR - invert 0
 *                  ESL_ERC_INVALID_LENGTH      - public key length is different from what is expected
 *  \pre            workSpace is initialized by esl_initWorkSpaceHeader(..)
 *  \context        TASK
 *  \reentrant      TRUE, for different workspaces
 *  \synchronous    TRUE
 *********************************************************************************************************************/
VSECPRIM_FUNC(eslt_ErrorCode) esl_generateSharedSecretEdDH(
  VSECPRIM_P2VAR_PARA(eslt_WorkSpaceEdDH) workSpace,
  VSECPRIM_P2CONST_PARA(eslt_Byte) publicKey,
  const eslt_Length publicKeyLength,
  VSECPRIM_P2VAR_PARA(eslt_Byte) sharedSecret,
  VSECPRIM_P2VAR_PARA(eslt_Length) sharedSecretLength);
# endif /* (VSECPRIM_EDDH_ENABLED == STD_ON) */
