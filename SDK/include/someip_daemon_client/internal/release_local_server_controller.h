 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file  someip_daemon_client/internal/release_local_server_controller.h
 *        \brief  ReleaseLocalServer API of SomeIpDaemonClient not requiring responses.
 *
 *      \details  Send release local server command not requiring responses from SomeIpDaemon.
 *
 *********************************************************************************************************************/

#ifndef LIB_SOMEIP_DAEMON_CLIENT_INCLUDE_SOMEIP_DAEMON_CLIENT_INTERNAL_RELEASE_LOCAL_SERVER_CONTROLLER_H_
#define LIB_SOMEIP_DAEMON_CLIENT_INCLUDE_SOMEIP_DAEMON_CLIENT_INTERNAL_RELEASE_LOCAL_SERVER_CONTROLLER_H_
...
/*!
 * \brief Class for release local server control command sent from application to SOME/IP daemon that require no
 * responses.
...
class ReleaseLocalServerController final {
 public:
  /*!
   ...
   * release local server control command to the SOME/IP daemon.
   ...
   * \steady FALSE
   */
  explicit ReleaseLocalServerController(MessageSender& message_sender) noexcept : message_sender_{message_sender} {}

  /*!
   * \brief Destructor of ReleaseLocalServerController.
   * \pre -
   * \context Shutdown
   * \reentrant FALSE
   * \steady FALSE
   */
  ~ReleaseLocalServerController() noexcept = default;

  ReleaseLocalServerController(ReleaseLocalServerController const&) = delete;
  auto operator=(ReleaseLocalServerController const&) -> ReleaseLocalServerController& = delete;
  ReleaseLocalServerController(ReleaseLocalServerController const&&) = delete;
  auto operator=(ReleaseLocalServerController const&&) -> ReleaseLocalServerController& = delete;

  /*!
   * \brief Sends request to release a local server to the SOME/IP daemon.
   * \details After calling this function, the SOME/IP daemon releases ownership of the local server from its skeleton,
   *  identified with the provided service instance id
   ...
   * - Send a ReleaseLocalServer command with the previously serialized request packet.
   * - Return the result.
   * \endinternal
   */
  auto operator()(ProvidedServiceInstanceType const& provided_service_instance) noexcept -> amsr::core::Result<void> {
    // PTP-B-SomeipDaemonClient-ReleaseLocalServerController_ReleaseLocalServer
    ...
    logger_.LogDebug([](ara::log::LogStream& s) { s << "Sending Release Local Server control command: "; }, {__func__},
                     {__LINE__});

    // PTP-E-SomeipDaemonClient-ReleaseLocalServerController_ReleaseLocalServer
 ...
 private:
  /*!
   * \brief ReleaseLocalServerController logger.
   */
  ...
  amsr::someip_daemon_client::internal::logging::AraComLogger logger_{
      ...
      amsr::core::StringView{"ReleaseLocalServerController"}};
  ...
  /*!
   * \brief ReleaseLocalServerController lock.
   */
  std::mutex lock_{};
  ...
  amsr::someipd_app_protocol::internal::IpcPacketSerializer serializer_{
      amsr::someipd_app_protocol::internal::MessageType::kReleaseLocalServer};
};
...
#endif  // LIB_SOMEIP_DAEMON_CLIENT_INCLUDE_SOMEIP_DAEMON_CLIENT_INTERNAL_RELEASE_LOCAL_SERVER_CONTROLLER_H_

/*!
 * \exclusivearea ::amsr::someip_daemon_client::internal::ReleaseLocalServerController::lock_
 *                Ensures that there is no concurrent read/write to the message_sender_ instance and to the memory
 *                buffers for the release local server controller
 *
 * \protects      ::amsr::someip_daemon_client::internal::ReleaseLocalServerController::serializer_
 *                ::amsr::someip_daemon_client::internal::ReleaseLocalServerController::message_sender_
 *
 * \usedin        ::amsr::someip_daemon_client::internal::ReleaseLocalServerController::operator()
 *
 * \length        LONG
