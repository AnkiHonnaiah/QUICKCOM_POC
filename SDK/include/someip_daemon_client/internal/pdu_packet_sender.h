 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file  someip_daemon_client/internal/pdu_packet_sender.h
 *        \brief  A class responsible for sending pdu messages.
 *        \details  This class sends messages regardless of their types (routing/control), to do that it makes use of
 *                  the passed message sender object.
 *
 *********************************************************************************************************************/

#ifndef LIB_SOMEIP_DAEMON_CLIENT_INCLUDE_SOMEIP_DAEMON_CLIENT_INTERNAL_PDU_PACKET_SENDER_H_
#define LIB_SOMEIP_DAEMON_CLIENT_INCLUDE_SOMEIP_DAEMON_CLIENT_INTERNAL_PDU_PACKET_SENDER_H_
/**********************************************************************************************************************
 *  INCLUDES
 *********************************************************************************************************************/
#include <mutex>
...
/*!
 * \brief A pdu packet sender class that makes use of the message sender to provide functionalities for sending all
 * different types of messages between application and SOME/IP daemon.
...
class PduPacketSender {
 public:
  /*!
   * \brief Type alias for message sender.
   */
  using MessageSender = typename TemplateConfiguration::MessageSenderType;

  /*!
   * \brief Constructor that initializes the pdu packet sender.
   ...
   * \steady FALSE
   */
  explicit PduPacketSender(MessageSender& message_sender) noexcept : message_sender_{message_sender} {}

  /*!
   * \brief Delete default constructor.
   */
  PduPacketSender() = delete;

  /*!
   * \brief Delete copy constructor.
   */
  PduPacketSender(PduPacketSender const&) = delete;

  /*!
   * \brief Delete move constructor.
   */
  PduPacketSender(PduPacketSender&&) = delete;

  /*!
   * \brief Delete copy assignment.
   */
  auto operator=(PduPacketSender const&) & -> PduPacketSender& = delete;

  /*!
   * \brief Delete move assignment.
   */
  auto operator=(PduPacketSender&&) & -> PduPacketSender& = delete;
  ...
  virtual ~PduPacketSender() noexcept = default;
   ...
   * \param[in] instance_id PDU service instance identifier.
   * \param[in] packet A memory buffer pointer containing a Pdu routing message.
   ...
   *   - Serialize the ipc headers for the PDU routing packet.
   *   - Send the PDU routing packet.
   * - Return the result.
   * \endinternal
   */
  auto operator()(amsr::someip_protocol::internal::InstanceId const& instance_id,
    ...
    // PTP-B-SomeipDaemonClient-PduPacketSender_SendPduMessage
    ...
    if (packet != nullptr) {
      ...
      logger_.LogDebug(
          [&instance_id, &packet_size](ara::log::LogStream& s) {
            s << "Sending PDU packet with Instance ID: 0x" << instance_id
    ...
    // PTP-E-SomeipDaemonClient-PduPacketSender_SendPduMessage
    return result;
  }

 private:
  /*!
   * \brief PduPacketSender logger.
   */
  ...
  amsr::someip_daemon_client::internal::logging::AraComLogger logger_{
      ...
      amsr::core::StringView{"PduPacketSender"}};
  ...
  amsr::someipd_app_protocol::internal::IpcPacketSerializer serializer_{
      amsr::someipd_app_protocol::internal::MessageType::kPdu};
};
...
#endif  // LIB_SOMEIP_DAEMON_CLIENT_INCLUDE_SOMEIP_DAEMON_CLIENT_INTERNAL_PDU_PACKET_SENDER_H_

/*!
 * \exclusivearea ::amsr::someip_daemon_client::internal::PduPacketSender::lock_
 *                Ensures that there is no concurrent read/write to the message_sender_ instance and to the memory
 *                buffers for the pdu packet sender
 *
 * \protects      ::amsr::someip_daemon_client::internal::PduPacketSender::serializer_
 *                ::amsr::someip_daemon_client::internal::PduPacketSender::message_sender_
 *
 * \usedin        ::amsr::someip_daemon_client::internal::PduPacketSender::operator()
 *
 * \length        LONG
 *                Serializing routing message and sending it to the message_sender_
 * \endexclusivearea
 */
