...
#define NTFRSLT_E_TIMEOUT_A                                                                              \
  0x02 /* Error notification:                                                                            \
                - timer N_Ar/N_As (according to ISO specification [ISONM]) has passed its time-out value \
          N_Asmax/N_Armax. This value can be issued to service user on both the sender and receiver side. */
#define NTFRSLT_E_TIMEOUT_BS                                                                                 \
  0x03 /* Error notification:                                                                                \
...
#define NTFRSLT_E_ABORT                                                                                       \
  0x09 /* Error notification:                                                                                 \
                - Flow control (FC) N_PDU with FlowStatus = ABORT received.                                   \
                  It indicates an abort of a transmission. A possible reason for this is that the receiver is \
          currently busy and can not take the request at that point in time. */
#define NTFRSLT_E_NO_BUFFER                                                                                  \
  0x0A /* Error notification:                                                                                \
...
#define NTFRSLT_E_CANCELATION_OK                 \
  0x0B /* Action has been successfully finished: \
                - Requested cancellation has been executed. */
#define NTFRSLT_E_CANCELATION_NOT_OK                                                                                  \
  0x0C /* Error notification:                                                                                         \
                - Due to an internal error the requested cancelation has not been executed. This will happen e.g., if \
          the to be canceled transmission has been executed already. */
...
#define NTFRSLT_E_PARAMETER_NOT_OK                                                                        \
  0x0E                       /* The request for the change of the parameter did not complete successfully \
                              */
 ...
 */
typedef struct {
  SduDataPtrType SduDataPtr; /**< SduDataPtr */
  PduLengthType SduLength;   /**< SduLength */
} PduInfoType;
...
typedef uint16
    PNCHandleType; /* Neccessary for partial network. Deviation from SWS_COMTYPE_00036 for ASR Version <= 4.4.0 */
...
typedef enum {
  ...
  TP_DATARETRY, /* TP_DATARETRY indicates that this API call shall copy already copied data in order to recover from
                   an error. In this case TxTpDataCnt specifies the offset of the first byte to be copied by the API
                   call. */
  TP_CONFPENDING
} TpDataStateType;
 ...
 */
typedef struct {
  TpDataStateType TpDataState; /**< TpDataState */
  PduLengthType TxTpDataCnt;   /**< TxTpDataCnt */
} RetryInfoType;
