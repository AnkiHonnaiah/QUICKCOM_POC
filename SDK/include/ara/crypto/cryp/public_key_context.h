 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/*!        \file  ara/crypto/cryp/public_key_context.h
 *        \brief  Generalized interface of Public Key algorithm Context.
 *
 *********************************************************************************************************************/

#ifndef LIB_LIBARA_INCLUDE_ARA_CRYPTO_CRYP_PUBLIC_KEY_CONTEXT_H_
#define LIB_LIBARA_INCLUDE_ARA_CRYPTO_CRYP_PUBLIC_KEY_CONTEXT_H_
 ...
 * \vpublic
 */
class PublicKeyContext : public KeyedContext {
 public:
   ...
   *       \b must be set before the loading of a key value (via call of the \c CryptoContext::Reset())!
   * \context ANY
   * \pre -
   * \threadsafe TRUE
   * \error SecurityErrorDomain::kIncompatibleObject  if provided key object is incompatible with
   *                an algorithm assigned to this public key context
   * \error SecurityErrorDomain::kBadObjectReference  if provided public key object references to
   *                an instance of the domain parameters different from the one loaded to the context,
   *                i.e. if the  COUID of the domain parameters in the context (see \c GetParametersUid())
   *                is not equal to the COUID referenced from the public key object
...
#endif  // LIB_LIBARA_INCLUDE_ARA_CRYPTO_CRYP_PUBLIC_KEY_CONTEXT_H_
