 ...
 *         \unit  VaCommonLib::ErrorHandling::ErrorCode
 *
 *********************************************************************************************************************/
...
class ErrorCode final {
  ...
  // VECTOR Next Construct VectorC++-V0.1.5: MD_VAC_V0-1-5_unusedTypeDeclaration
  /*!
   * \brief Helper class to verify that non-ErrorCode enums cannot construct ErrorCodes.
   * \tparam T An enum to verify.
   */
  template <typename T>
  class HasMakeErrorCode {
    /*!
     * \brief Returns true if enum T is allowed to construct an ErrorCode.
     * \tparam U An enum to verify.
     */
    template <typename U, typename = decltype(MakeErrorCode(std::declval<U>(),
                                                            std::declval<typename ErrorDomain::SupportDataType>(),
                                                            std::declval<char const*>()))>
    /*!
     * \brief Returns true if enum T is of ErrorCode type.
     * \returns An std::true_type.
     */
    static auto VerifyEnum(std::int32_t) noexcept -> std::true_type;
    /*!
     * \brief Returns false if enum T is of non-ErrorCode type.
     * \returns An std::false_type.
     */
    template <typename>
    static auto VerifyEnum(...) noexcept -> std::false_type;

   public:
    /*!
     * \brief Holds the value returned by VerifyEnum<T>.
     */
    static constexpr bool const value{decltype(VerifyEnum<T>(0))::value};
  };
 ...
 public:
  ...
  template <typename EnumT,
            typename = typename std::enable_if<std::is_enum<EnumT>::value && HasMakeErrorCode<EnumT>::value>::type>
  ...
  /*!
   * \brief     Construct a new ErrorCode instance with parameters.
   * \param[in] value Domain-specific error code value.
   ...
   * \endspec
   */
  StringView Message() const noexcept { return StringView{Domain().Message(Value())}; }  // VCA_VAC_VALID_DOMAIN_ACCESS
   ...
   * \endspec
   */
  void ThrowAsException() const noexcept(false) { Domain().ThrowAsException(*this); }  // VCA_VAC_VALID_DOMAIN_ACCESS

  /*!
   * \brief     Sets the support data field for this error code.
   * \param[in] support_data The support data to set.
   * \pre       -
