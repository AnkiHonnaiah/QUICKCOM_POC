 ...
 * \trace CREQ-VaCommonLib-Optional
 * \vpublic
 */
template <class T>
 ...
 * \details   Initializes the contained value as if direct-non-list-initializing an object of type T with the
              expression std::forward<U>(v).
 * \param[in] v The value to construct the Optional from.
   * \spec
   *   requires true;
   * \endspec
 * \vpublic
 */
  ...
  Optional(U&& v) noexcept : Optional(in_place, std::forward<U>(v)) {}

  // VECTOR Next Construct VectorC++-V12.1.4: MD_VAC_V12-1-4_constructorShallBeDeclaredExplicit
  /*!
   * \brief   Copy Constructor from default value.
  ...
  template <
      ...
      typename = std::enable_if_t<
          ...
          (!std::is_constructible<T, Optional<U>&&>::value) && (!std::is_constructible<T, Optional<U> const&>::value) &&
  ...
  Optional(Optional<U> const& rhs) noexcept : Optional() {
    if (rhs.has_value()) {
      static_cast<void>(emplace(rhs.value()));
    }
  }
  ...
  /*!
   * \brief     Move Constructor from default value.
   * \param[in] rhs The value to construct the Optional from
   * \spec
   *   requires true;
   * \endspec
   * \vpublic
  ...
  template <
      ...
      typename = std::enable_if_t<
          ...
          (!std::is_convertible<Optional<U>&&, T>::value) && (!std::is_convertible<Optional<U> const&, T>::value) &&
          (!std::is_convertible<Optional<U> const&&, T>::value)>>
  Optional(Optional<U>&& rhs) noexcept : Optional() {
    if (rhs.has_value()) {
      static_cast<void>(emplace(std::move(rhs.value())));
    }
  }
   ...
   * \param[in] args The arguments to pass to the values constructor.
   * \vpublic
   */
  template <class... Args>
   ...
   * \details   If an exception is raised by the move constructor in T the process is terminated.
   * \param[in] rhs The moved Optional from which to construct a new Optional.
   * \vpublic
   */
   ...
   * \details   If an exception is raised by the constructor or assignment in T the process is terminated.
   * \param[in] rhs The Optional to move from.
   * \return    The copied Optional.
   * \vpublic
   */
   ...
   * \details   If an exception is raised by the constructor in T the process is terminated.
   * \param[in] args Arguments to pass to the underlying types constructor.
   * \pre       -
  ...
  // VECTOR Next Construct AutosarC++17_10-A13.3.1: MD_VAC_A13.3.1_forwardingFunctionsShallNotBeOverloaded
  /*!
   * \brief     Constructor function used to build the contained element with initializer list.
   * \details   If an exception is raised by the constructor in T the process is terminated.
   * \param[in] il Initializer list to be passed to the constructor of T.
   * \param[in] args Arguments to pass to the underlying types constructor.
   * \pre       -
   * \return    A reference to the underlying value.
   * \vpublic
  ...
  T& emplace(std::initializer_list<U> il, Args&&... args) noexcept {
    return this->data_.EmplaceRight(il, std::forward<Args>(args)...);
  }
   ...
   * \param[in] default_value The value to use in case ptr_ is empty.
   ...
   * \vpublic
   */
  constexpr T& operator*() & noexcept { return this->data_.RightUnsafe(); }
   ...
   * \vpublic
   */
  constexpr T const& operator*() const& noexcept { return this->data_.RightUnsafe(); }
   ...
   * \vpublic
   */
  constexpr T&& operator*() && noexcept { return std::move(this->data_).RightUnsafe(); }
  ...
  constexpr T const&& operator*() const&& noexcept {
    // VECTOR Next Line AutosarC++17_10-A18.9.3: MD_VAC_A18.9.3_moveWithConstArgument
    return std::move(this->data_.RightUnsafe());
  }

  // VECTOR Next Construct AutosarC++17_10-A15.5.1: MD_VAC_A15.5.1_explicitNoexceptIfAppropriate
  /*!
   * \brief          Exchange the state of this Optional with other Optional.
   * \param[in, out] other The other Optional to swap with.
   * \pre            -
   * \spec
   *   requires true;
   * \endspec
 ...
 private:
  ...
  FRIEND_TEST(UT__Optional, Construct);
  /*! Friend declaration for testing purposes */
  FRIEND_TEST(UT__Optional, Value);
  /*! Friend declaration for testing purposes */
  FRIEND_TEST(UT__Optional, Size);
};
 ...
 * \param[in] value The value to construct optional object with.
 * \pre       -
 * \spec
 *   requires true;
 * \endspec
 ...
 * \param[in] args Arguments to be passed to the constructor of T.
 * \pre       -
 * \spec
 *   requires true;
 * \endspec
 ...
 * \param[in,out] lhs Optional object to be swapped.
 * \param[in,out] rhs Optional object to be swapped.
 * \spec
 *   requires true;
 * \endspec
 * \vpublic
