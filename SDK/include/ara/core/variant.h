 ...
 * \vpublic
 */
// TODO(ofawzi): this struct will be final in HUN-3477 to solve AutosarC++17_10-A12.4.1 bauhaus violation.
struct monostate {};  // VECTOR SL AutosarC++17_10-A12.4.1: MD_VAC_A12.4.1_compileTimeResolution
 ...
 * \tparam Types Type of the variants that are compared.
 * \vprivate
 */
template <typename... Types>
class VariantCompare {
 ...
 public:
   ...
   * \param[in] v1 The first Variant to compare.
   * \param[in] v2 The second Variant to compare.
   * \pre       -
   * \spec
   *   requires true;
   * \endspec
  ...
  VariantCompare(variant_t v1, variant_t v2) noexcept : v1_{v1}, v2_{v2} {}
   ...
   */
  bool is_smaller() const&& noexcept {
    bool ret_value{false};
    if (v1_.index() < v2_.index()) {
      ret_value = true;
 ...
 private:
   ...
   * \tparam       Cand The type to be compared with.
   * \pre          -
   * \return       True if the comparison function returns true.
   * \pre          Both variants are valid and have values of the same type.
   */
   ...
   * \tparam       Comp The function that compares both items.
   * \tparam       Cand The type to be compared with.
   * \tparam       Cand2 The type to be compared with.
   * \tparam       Cands The type to be compared with.
   * \pre          -
  ...
  bool compare_internal() const noexcept {
    bool ret_value{false};
    if (ara::core::holds_alternative<Cand>(v1_)) {  // VCA_VAC_VALID_ARGUMENT
 ...
 * \tparam    Visitor The visitor type.
 * \tparam    Variant The variant type.
 * \tparam    T The type should be get from.
 * \tparam    Types The type should be get from.
 * \param[in] visitor The visitor accepting the stored value.
 * \param[in] v The visited Variant.
 * \pre       -
 * \spec
 *   requires true;
 * \endspec
...
auto TryVisitVariant(Visitor&& visitor, Variant&& v) noexcept -> Ret {
  ...
  return (maybe_pointer != nullptr) ? std::forward<Visitor>(visitor).operator()(*maybe_pointer)
                                    : TryVisitVariant<Ret, Visitor, Variant, Types...>(  // VCA_VAC_VALID_ARGUMENT
                                          std::forward<Visitor>(visitor), std::forward<Variant>(v));
}
...
auto VisitVariant(Visitor&& visitor, Variant&& v) noexcept -> decltype(visitor.operator()(*ara::core::get_if<0>(&v))) {
 ...
 * \tparam Variant The variant type of the vistor.
 * \vprivate
 */
...
class SwapWithVisitor {
 public:
  /*!
   * \brief     Constructor.
   * \param[in] other A variant to construct SwapWithVisitor.
   * \spec
   *   requires true;
   * \endspec
   */
   ...
   * \tparam         T The type of the other.
   * \param[in, out] v The other to be swapped from.
   * \pre            -
   * \spec
   *   requires true;
   * \endspec
  ...
  void operator()(T& v) noexcept {
    using std::swap;
    swap(v, get<T>(other_));  // VCA_VAC_VALID_MEMBER_VARIABLE_ACCESS, VCA_VAC_STD_FUNC_FULFILL_CONTRACT
  }
 ...
 *          Alternative types must consider the following:
 *           All types must meet the "C++ Named Requirements: Destructible".
 *           Variant does not support the following types:
 *           -References
 *           -Arrays
 *           -The type void
 *           -Empty Variants (Variant<ara::core::monostate> can be used instead).
 *           A Variant does not support volatile-qualified alternatives.
 *           A Variant is not allowed to have more than one alternative of the same type, but is allowed to
 *           have different const-qualified alternatives of the same type with some limitations. For example
 *           Variant<string, const string> v{"abc"} is ill-formed since both alternative types
 *           have an equally viable constructor for the argument.
 * \tparam  Xs The containable types of the alternatives. See details.
...
class Variant final {
  ...
  static_assert(!vac::language::compile_time::has_duplicate<std::remove_const_t<Xs>...>::value,
                "Multiple alternative types have an equally viable constructor for the argument!");

  /*!
   * \brief  Collection of traits useful for Variant.
   * \tparam X Value type that to be returned.
   * \vprivate
   */
  template <typename X>
  class VariantTrait {
   public:
    ...
    /*!
     * \brief The index of the alternative of type X.
     */
    static constexpr std::size_t const index{vac::language::compile_time::get_index<X, Xs...>()};
 ...
 public:
  ...
  Variant() noexcept : storage_{}, variant_index_{Trait::index} {
    new (&storage_) Default();
  }

  /*!
   * \brief     Constructor from universal reference type.
   * \param[in] value The value that is stored inside the Variant.
   * \tparam    X Type to be stored inside the Variant. It shall not be tagged as in_place_type_t nor in_place_index_t.
   *            It must be one of the alternatives types and it must be constructable with value as argument.
   * \tparam    Trait Trait of stored type inside the Variant.
   * \pre       -
   * \spec
   *   requires true;
   * \endspec
   * \vpublic
   */
  template <typename X, typename Trait = VariantTrait<X>, typename = std::enable_if_t<!Trait::is_same_variant>,
            typename = std::enable_if_t<!is_index_spec<std::decay_t<X>>::value>,
            typename = std::enable_if_t<!is_type_spec<std::decay_t<X>>::value>,
            typename = std::enable_if_t<std::not_equal_to<std::size_t>()(variant_npos, Trait::accepted_index)>,
            typename AcceptedType = variant_alternative_t<Trait::accepted_index, Variant>,
            typename = std::enable_if_t<std::is_constructible<AcceptedType, X>::value>>
  explicit Variant(X&& value) noexcept : Variant(in_place_type_t<AcceptedType>(), std::forward<X>(value)) {}

  // VECTOR Next Construct AutosarC++17_10-M4.5.1: MD_VAC_M4.5.1_boolOperandInNew
  /*!
   * \brief     In-place constructor.
   * \param[in] args Arguments of construction.
   * \tparam    X That shall be emplace into the Variant.
   * \tparam    Trait Trait of stored type inside the Variant
   * \pre       -
   * \spec
   *   requires true;
  ...
  template <typename X, typename Trait = VariantTrait<X>, typename... Args,
            typename = std::enable_if_t<std::is_constructible<X, Args...>::value &&
                                        std::not_equal_to<std::size_t>()(variant_npos, VariantTrait<X>::index)>>
  explicit Variant(in_place_type_t<X>, Args&&... args) noexcept : storage_{}, variant_index_{Trait::accepted_index} {
    // VECTOR Next Line AutosarC++17_10-M5.2.12: MD_VAC_M5.2.12_arraytoPointerDecay
    new (&storage_) typename Trait::decayed(std::forward<Args>(args)...);  // VCA_VAC_STD_FUNC_FULFILL_CONTRACT
  }
   ...
   * \vpublic
   */
  Variant(Variant const& other) noexcept : storage_{}, variant_index_{other.variant_index_} {
   ...
   * \vpublic
   */
  Variant(Variant&& other) noexcept : storage_{}, variant_index_{other.variant_index_} {
   ...
   * \vpublic
   */
  ~Variant() noexcept { destroy_storage<Xs...>(variant_index_); }

  // VECTOR Next Construct AutosarC++17_10-A13.3.1: MD_VAC_A13.3.1_forwardingFunctionsShallNotBeOverloaded
  /*!
   * \brief     Copy assignment.
   * \param[in] other The Variant that is to be copied.
   * \pre       -
   * \spec
   *   requires true;
   * \endspec
  ...
  Variant& operator=(Variant const& other) & noexcept {
    if (this != &other) {
      // variant_index_ will be npos in prepare_assignment
      prepare_assignment();
      copy_storage<Xs...>(other.variant_index_, other.storage_);
   ...
   * \param[in] other The Variant that is to be moved.
   * \pre       -
   * \spec
   *   requires true;
   * \endspec
   ...
   * \tparam    X Type to be stored inside the Variant. X must be assignable to at least one of the alternatives.
   * \tparam    Trait Trait of stored type inside the Variant.
   * \tparam    AcceptedType Type which can be accepted.
   * \param[in] value The value that is to be assigned.
   * \pre       -
   * \spec
   *   requires true;
   * \endspec
  ...
  Variant& operator=(X&& value) & noexcept {
   ...
   * \tparam    T Type to be assigned to.
   * \tparam    Args Types of the value that is to be moved.
   * \param[in] args The value that is to be moved.
   * \pre       -
   * \spec
   *   requires true;
   * \endspec
  ...
  template <typename T, typename... Args,
            typename = std::enable_if_t<std::is_constructible<T, Args...>::value &&
                                        std::not_equal_to<std::size_t>()(variant_npos, VariantTrait<T>::index)>>
  ...
  /*!
   * \brief     Assign value in place.
   * \tparam    I The amount of types to be passed in.
   * \tparam    Args Types of the value that is to be moved.
   * \param[in] args The value that is to be moved.
   * \pre       -
   * \spec
  ...
  template <size_t I, typename... Args,
            typename = std::enable_if_t<std::is_constructible<variant_alternative_t<I, Variant>, Args...>::value>>
   ...
   * \vpublic
   */
  // VECTOR NC AutosarC++17_10-M9.3.3: MD_VAC_M9.3.3_methodCanBeDeclaredStatic
  auto valueless_by_exception() const noexcept -> bool { return false; }
   ...
   *   requires true;
   * \endspec
   * \return A pointer to the stored value, or nullptr if Variant does not hold T.
   * \vprivate
   */
  template <typename T>
  auto get_if() noexcept -> std::add_pointer_t<T> {
    std::add_pointer_t<T> ret_value{nullptr};
    if (ara::core::holds_alternative<T>(*this)) {
  ...
  auto get_if() const noexcept -> std::add_pointer_t<T const> {
    ...
    if (ara::core::holds_alternative<T>(*this)) {  // VCA_VAC_VALID_ARGUMENT
      ret_value = get_unsafe<T>(storage_);
    }
    return ret_value;
  }
   ...
   * \param[in, out] other Variant to swap with.
   * \pre            -
   * \spec
   *   requires true;
   * \endspec
  ...
  void swap(Variant& other) noexcept {
    if (this->index() == other.index()) {
      ...
      other = std::move(*this);  // VECTOR SL AutosarC++17_10-A12.8.3: MD_VAC_A12.8.3_movedFromObjectVariant
 ...
 private:
   ...
   * \brief     Gets the stored value cast to the specified type without any checks.
   * \tparam T  Type to be assigned to.
   * \param[in] store The storage from which to get the value from.
   * \pre       -
   * \spec
   *   requires true;
   * \endspec
   ...
   * \brief  Gets the stored value cast to the specified type without any checks.
   * \tparam T Type to be assigned to.
   * \param  store The storage from which to get the value from.
   * \pre    -
   * \spec
   *   requires true;
   * \endspec
   ...
   * \tparam    Hd Type to be assigned to.
   * \param[in] store The data storage from which to move.
   * \pre       -
   * \spec
   *   requires true;
   * \endspec
  ...
  void move_storage(std::size_t, Storage&& store) noexcept {
   ...
   * \brief     Move the stored value from that to this.
   * \tparam    Hd Type to be assigned to.
   * \tparam    Tl1 Type to be assigned to.
   * \tparam    Tl Type to be assigned to.
   * \param[in] offset The type offset.
   * \param[in] store The data storage from which to move.
   * \pre       -
   * \spec
   *   requires true;
   ...
   */
  template <typename Hd>
  void copy_storage(std::size_t, Storage const& storage) noexcept {
    new (&storage_) Hd(*get_unsafe<Hd>(storage));  // VCA_VAC_STD_FUNC_FULFILL_CONTRACT
  }
   ...
   */
  template <typename Hd, typename Tl1, typename... Tl>
  void copy_storage(std::size_t offset, Storage const& store) noexcept {
    if (offset == 0) {
  ...
  void destroy_storage(std::size_t) noexcept {
    get_unsafe<Hd>(storage_)->~Hd();  // VCA_VAC_CHECKED_ACCESS_TO_VALID_POINTER
  }
...
template <typename T, typename... Types>
bool holds_alternative(Variant<Types...> const& v) noexcept {
  static_assert(vac::language::compile_time::any_in<T, Types...>() ||
                    vac::language::compile_time::any_in<std::decay_t<T>, Types...>(),
                "T is not contained in the Variant!");
  return vac::language::compile_time::get_index<T, Types...>() == v.index();
}

/*!
 * \brief     Get Variant value from index as pointer.
 * \tparam    N The index of the alternative to get the value from.
 * \tparam    Types The types that the Variant holds.
 * \param[in] v The Variant which value shall be returned.
 * \pre       -
 * \spec
 *   requires true;
 * \endspec
...
auto get_if(Variant<Types...> const* v) noexcept -> std::add_pointer_t<T const> {
  std::add_pointer_t<T const> ret_value{nullptr};
  if (v != nullptr) {
    ret_value = v->template get_if<T>();  // VCA_VAC_VALID_NONSTATIC_METHOD
  }
  return ret_value;
}
...
variant_alternative_t<N, Variant<Types...>> const& get(Variant<Types...> const& v) noexcept {
  static_assert(N < sizeof...(Types), "Out of bounds access!");
  return get<variant_alternative_t<N, Variant<Types...>>, Types...>(v);  // VCA_VAC_VALID_RETURN_VALUE
}
...
T& get(Variant<Types...>& v) noexcept {
  ...
  return *result;  // VCA_VAC_VALID_RETURN_VALUE
}
...
T const& get(Variant<Types...> const& v) noexcept {
  ...
  return *result;  // VCA_VAC_VALID_RETURN_VALUE_RUN_TIME_CHECK
}
 ...
 */
template <typename... Types>
bool operator==(Variant<Types...> const& v1, Variant<Types...> const& v2) noexcept {
  return detail::VariantCompare<Types...>{v1, v2}.equal();  // VCA_VAC_VALID_ARGUMENT
}
...
template <typename... Types>
bool operator<(Variant<Types...> const& v1, Variant<Types...> const& v2) noexcept {
  return detail::VariantCompare<Types...>{v1, v2}.is_smaller();  // VCA_VAC_VALID_ARGUMENT
}
 ...
 * \tparam    Visitor Type of the visitor.
 * \tparam    Types The types that the Variant holds.
 * \param[in] visitor The visitor, a functor accepting all variants.
 * \param[in] v The Variant to visit.
 * \pre       -
 * \spec
 *   requires true;
 * \endspec
...
auto visit(Visitor&& visitor, Variant<Types...>& v) noexcept
 ...
 */
template <typename Visitor, typename... Types>
auto visit(Visitor&& visitor, Variant<Types...> const& v) noexcept
 ...
 * \tparam         Types The types that the Variant holds.
 * \param[in, out] lhs Variant object to be swapped
 * \param[in, out] rhs Variant object to be swapped
 * \pre            -
 * \spec
 *   requires true;
 * \endspec
 ...
 */
template <typename... Types>
inline auto swap(Variant<Types...>& lhs, Variant<Types...>& rhs) noexcept
