 ...
 *                         visivg    STORYC-7448      Support of HMAC-SHA-384
 ...
 * 07.00.00 2022-12-15     viseag    SEC-5046         Support of HKDF HMAC SHA384
 *                                   ESCAN00113427    Missing information in API description of vSecPrim_initHash().
 * 07.01.00 2023-02-07     vismhh    HSM-3532         Finite Execution Time Analysis for Crypto
 *          2023-03-03     viseag    SEC-5813         Improve robustness of processing of elliptic curve domain parameters
 *                         viseag    SEC-5945         Refactor modulo reduction and Barrett optimization
 * 08.00.00 2023-03-14     olapointe SEC-5935         Support One Step HKDF based on SHA512
 * 08.01.00 2023-04-25     lmangold  SEC-5674         Support (Ed)DH based on X448
 *                         lmangold  SEC-5678         Support Ed448 Signature Generation
 *                         lmangold  SEC-5680         Support Ed448 Signature Verification
 * 08.02.00 2023-07-26     olapointe SEC-6283         Support variable output length for SHAKE family
 *                         lmangold  SEC-6308         Support HashMac using SHA512
 *                         lmangold  SEC-6304         Support HKDF HMAC SHA256 and SHA384 using a precomputed key
 *                         lmangold  SEC-6429         Support EcP key checks
 *                         vismwe    ESCAN00115096    Failing CAVP test vectors due to insufficient padding check for RSASSA-PKCS1-v1.5 Signature Verify
 * 08.03.00 2023-08-24     lmangold  SEC-6732         Support Edwards25519 and Edwards448 key checks
 *                         vismhh    ESCAN00115546    Added file for backwards compatibility
 * 08.04.00 2023-08-24     lmangold  SEC-6689         Update to new QAC (QAC 2022.2)
 *          2023-11-13     lmangold  ESCAN00116057    actBNReduce does not return the correct remainder in rare cases
 *                         lmangold  ESCAN00115992    AES GCM fails to calculate/verify tag correctly due to a insufficient shift
 *          2023-11-14     vismhh    ESCAN00115758    Corrected location of const array
 **********************************************************************************************************************/
...
# define VSECPRIM_SW_MAJOR_VERSION                                    (8u)
# define VSECPRIM_SW_MINOR_VERSION                                    (4u)
...
# define VSECPRIM_VENDOR_ID                                           (30u)
# define VSECPRIM_MODULE_ID                                           (255u)
...
/**********************************************************************************************************************
 *  vSecPrim_getVersionInfo()
 *********************************************************************************************************************/
/*! \brief       Returns the version information
 *  \details     vSecPrim_getVersionInfo() returns version information, vendor ID and AUTOSAR module ID of the component.
 ...
 *  \synchronous TRUE
 *********************************************************************************************************************/
extern VSECPRIM_FUNC(eslt_ErrorCode) vSecPrim_getVersionInfo(
  VSECPRIM_P2VAR_PARA(vSecPrim_VersionInfoType) versioninfo);
 ...
 *                                      Hint: outputLengthFlag has no influence on the SHAKE algorithms, since the
 *                                            digest length is variable and automatically set to the buffer length.
 *                                            Hence, you must give a valid flag as parameter but which one is not
 *                                            important.
 *  \return         ESL_ERC_PARAMETER_INVALID    input parameter is NULL
 *                  ESL_ERC_INPUT_INVALID        value of hashID or of outputLengthFlag invalid
 *                  ESL_ERC_WS_TOO_SMALL         workspace too small
 *                  ESL_ERC_NO_ERROR             otherwise
 *  \pre            workSpace is initialized by esl_initWorkSpaceHeader(..), in particular the watchdog function
 *                  pointer in the workspace header must be initialized.
 ...
 *                  If 'messageDigest' is not a null pointer:
 *                    If the hash function specified by workSpace->hashID belongs to the SHAKE family:
 *                      - the hash function computes a digest of size *messageDigestLength and the digest
 *                        is written to 'messageDigest'.
 *                    Else (workSpace->hashID does not belong to the SHAKE family):
 *                      - this function calls the hash function finalize routine specified by workSpace->hashID
 *                        and returns the hash value.
 *                      - if *messageDigestLength is shorter than the algorithm specific digest size
 *                        and if truncation is allowed, the truncated digest is written to 'messageDigest'.
 *                  Else ('messageDigest' is a null pointer):
 *                    this function writes the number of required bytes to store the (untruncated) digest to
 *                    *messageDigestLength and does not compute the hash value.
 *                    In this case, the function can be called again.
 *  \param[in,out]  workSpace              (in)  Hash context buffer.
 *                                         (out) Updated hash context structure.
 *  \param[in,out]  messageDigestLength    (in)  Pointer to buffer, storing the length of messageDigest.
 *                                               If hashID corresponds to a SHAKE algorithm, the length of the
 *                                               ouput digest is set to value stored in this buffer.
 ...
 *                    - exactly as many bytes as referenced by messageDigestLength if a SHAKE algorithm is used.
 *                    - at least as many bytes as referenced by messageDigestLength, else.
 *  \note           Please refer to the API descriptions in ESLib.h for further information about the specific
 *                  hash finalization APIs. They can be found under esl_finalize[hash function name](..) or
 *                  esl_getXOFDigestSHAKE(..) for the SHAKE family.
 *  \context        TASK
 *  \reentrant      TRUE, for different workspaces
 *  \synchronous    TRUE
 *********************************************************************************************************************/
