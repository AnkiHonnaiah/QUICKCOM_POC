 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  SpeedMonitorExecutable/src/amsr/ipc_binding/internal/target_dependent/initializer.cpp
...
auto Initializer::Initialize(ReactorInterface& reactor,
  ...
  static_cast<void>(remote_server_manager);

  InitializeProvidedServiceInstances(
      reactor,
      binding_core.GetServiceDiscovery(),           // VCA_IPCB_LIFECYCLE_MANAGER_XF_LIFETIME
      binding_core.GetConnectionManagerSkeleton(),  // VCA_IPCB_LIFECYCLE_MANAGER_XF_LIFETIME
      binding_core.GetSkeletonRouter(),             // VCA_IPCB_LIFECYCLE_MANAGER_XF_LIFETIME
      trace_sink_factory, runtime_config);
} // VCA_IPCB_STD_UNIQUE_POINTER_DESTRUCTOR


void Initializer::InitializeProvidedServiceInstances(ReactorInterface& reactor,
  ...
  // Service Config for Service Interface '/ServiceInterfaces/PkgSpeedLimitService/Latest/SpeedLimitService'
  {
    ...
    logger_.LogDebug(
        ...
        s << "Initializing ProvidedServiceInstances of ServiceInterfaceDeployment '" << service_shortname_path << "'.";
        },
        Logger::LogLocation{static_cast<char const*>(__func__), __LINE__});
    ...
    using SkeletonXf = ::amsr::ipc_binding_transformation_layer::internal::gen_services::gen_speedlimitservice::SpeedLimitServiceSkeletonXf;

    ::amsr::ipc_binding_transformation_layer::internal::SkeletonXfFactoryInitializer::Initialize<
        SkeletonXf, Skeleton::SkeletonBackendFactoryRegistry::BaseType>(
        reactor,
        service_discovery,
        connection_manager_skeleton,
        skeleton_router,
        *(Skeleton::GetSkeletonFactoryRegistry()),
        trace_sink_factory,
        service_config,
        runtime_config.GetRuntimeProcessingMode());
